...
function decompileCbxBorderRadius() {
    roleList.forEach(role => {
        console.log(role, settings.cbxBorder[`${role}`].active);
        if (settings.cbxBorder[`${role}`].active === true) {
            document.querySelectorAll(`.cbx.${role}`).forEach(cbx => {
...
function decompileMsgEmotes() {
    roleList.forEach(role => {
        if (settings.msgEmotes[`${role}`].active === true) {
            document.querySelectorAll(`.cbx.${role}`).forEach(cbx => {
                var msg = cbx.querySelector(".msg");
                if (settings.msgEmotes[`${role}`].value.auto === true) {
                    msg.style.setProperty("--msgEmoteHeight", `${settings.msgFont[`${role}`].value.size}px`);
                } else if (settings.msgEmotes[`${role}`].value.auto === false) {
                    msg.style.setProperty("--msgEmoteHeight", `${settings.msgEmotes[`${role}`].value.height}px`)
                }
                
                msg.style.setProperty("--msgEmoteVPos", settings.msgEmotes[`${role}`].value.linePos);
            });
        }
    });
}
...
function updateMsgEmotes(type) {
    if (type === "autosize") {
        if (event.target.getAttribute("data-active") === "true") {
            event.target.setAttribute("data-active", "false");
            settings.msgEmotes[`${activeRole}`].value.auto = false;
        } else {
            event.target.setAttribute("data-active", "true");
            settings.msgEmotes[`${activeRole}`].value.auto = true;
        }
    } else if (type === "vAlign") {
        Array.from(event.target.parentNode.children).forEach(elmnt => {
            elmnt.setAttribute("data-active", "false");
        ...
        event.target.setAttribute("data-active", "true");
        settings.msgEmotes[`${activeRole}`].value.linePos = event.target.getAttribute("data-value");
    }

    settings.msgEmotes[`${activeRole}`].value.height = document.querySelector("#msgEmoteSizeInp").value;
    ...
    var colorPreset = [${presetSettingList}],
        settings = {
            ...
            msgEmotes: {
                user: {
                    value: {
                        auto: ${settings.msgEmotes.user.value.auto},
                        height: ${settings.msgEmotes.user.value.height}, // In px
                        linePos: "${settings.msgEmotes.user.value.linePos}" // top/middle/bottom
                    },
                    active: ${settings.msgEmotes.user.active}
                },
                sub: {
                    value: {
                        auto: ${settings.msgEmotes.sub.value.auto},
                        height: ${settings.msgEmotes.sub.value.height}, // In px
                        linePos: "${settings.msgEmotes.sub.value.linePos}" // top/middle/bottom
                    },
                    active: ${settings.msgEmotes.sub.active}
                },
                vip: {
                    value: {
                        auto: ${settings.msgEmotes.vip.value.auto},
                        height: ${settings.msgEmotes.vip.value.height}, // In px
                        linePos: "${settings.msgEmotes.vip.value.linePos}" // top/middle/bottom
                    },
                    active: ${settings.msgEmotes.vip.active}
                },
                mod: {
                    value: {
                        auto: ${settings.msgEmotes.mod.value.auto},
                        height: ${settings.msgEmotes.mod.value.height}, // In px
                        linePos: "${settings.msgEmotes.mod.value.linePos}" // top/middle/bottom
                    },
                    active: ${settings.msgEmotes.mod.active}
                }
            },
            anmTiming: {
                user: {
