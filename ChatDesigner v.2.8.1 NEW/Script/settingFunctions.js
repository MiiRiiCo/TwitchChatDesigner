...
function openNavMenu(item) {
    for (var i = 0; i < document.querySelectorAll(".navMenuWrapper").length; i++) {
        ...
        document.querySelectorAll(".openMenuButton")[i + 1].setAttribute("data-active", "false");
    }
    
    if (menuOpen === item) {
        document.querySelectorAll(".navMenuWrapper")[item].setAttribute("data-open", "false");
        document.querySelectorAll(".openMenuButton")[item + 1].setAttribute("data-active", "false");
        menuOpen = undefined;
    } else {
        document.querySelectorAll(".navMenuWrapper")[item].setAttribute("data-open", "true");
        document.querySelectorAll(".openMenuButton")[item + 1].setAttribute("data-active", "true");
        menuOpen = item;
    }
}
...
function openSetting(num) {
    if (num === openSettingNum) {
        ...
        document.querySelectorAll(".openSettingButton")[openSettingNum].setAttribute("data-active", "false");
        ...
        document.querySelectorAll(".openSettingButton")[openSettingNum].setAttribute("data-active", "true");
    }
}
...
function addPreset() {
    ...
    document.querySelectorAll(".presetList").forEach(list => {
        var newPreset = document.createElement("button");
            ...
            newPreset.setAttribute("onclick", "useColorPreset()");
...
function insertPresetColor() {
    var presetContent = ``;

    for (var i = 0; i < colorPreset.length; i++) {
        presetContent += `<button class="colorPreset" onclick="useColorPreset()" style="background-color:${colorPreset[i].color}${dec2hex(colorPreset[i].opacity)}"><div class="removePreset"><img class="iconSVG" src="Icons/Diverse/Remove.svg"></div></button>`
    }
...
function useColorPreset() {
    ...
    event.target.parentNode.parentNode.parentNode.querySelector(".colorChangeOpacity").setAttribute("data-value", colorPreset[i].opacity);
...
function showDeletePreset() {
    if (event.target.parentNode.querySelector(".presetList").getAttribute("data-delete-show") === "false") {
        ...
        event.target.parentNode.querySelectorAll(".colorPreset").forEach(preset => {
            preset.setAttribute("onclick", "useColorPreset()");
        });
...
var currentPreset = 0,
    currentPresetStyle = 0;

function switchPreset(type, dir) {
    if (type === "main") {
        if (dir === "+") {
        ...
        if (dir === "+") {
            if (currentPresetStyle < presets[currentPreset].sets.length - 1) {
                currentPresetStyle += 1;
            } else {
                currentPresetStyle = 0;
            }
        } else if (dir === "-") {
            if (currentPresetStyle > 0) {
                currentPresetStyle -= 1;
            } else {
                currentPresetStyle = presets[currentPreset].sets.length - 1;
            }
        }
    }
    document.querySelector(".presetMainName").innerHTML = presets[currentPreset].name;
    document.querySelector(".presetColorName").innerHTML = presets[currentPreset].sets[currentPresetStyle].colorName;
    document.querySelector(".presetPreviewImg").src = `PresetImgs/${currentPreset}/${currentPresetStyle}.png`;
}

function useFullPreset() {
    var confirmPreset = confirm("Do you want to overwrite the current settings?");
    if (confirmPreset === true) {
        var savedStreamer = JSON.parse(JSON.stringify(settings.genStreamerList)),
            savedHyphens = JSON.parse(JSON.stringify(settings.othHyphens));

        settings = JSON.parse(JSON.stringify(presets[currentPreset].sets[currentPresetStyle].set));

        settings.genStreamerList = savedStreamer;
        settings.othHyphens = savedHyphens;
...
var settingClipboard = {
    type: "",
    value: ""
};

function settingCopy() {
    settingClipboard.type = event.target.parentNode.getAttribute("data-type");
    settingClipboard.value = JSON.parse(JSON.stringify(eval(`${event.target.parentNode.getAttribute("data-copy")}["${activeRole}"].value`)));
    
    document.querySelector("aside").setAttribute("data-setting-paste-active", "true");
}

function settingPaste() {
    if (event.target.parentNode.getAttribute("data-type") === settingClipboard.type) {
        var confirmPaste = confirm("Do you want to overwrite the current settings?");
        if (confirmPaste === true) {
            eval(`${event.target.parentNode.getAttribute("data-copy")}`)[`${activeRole}`].value = settingClipboard.value;
            loadingData();
            decompileCollection();
        }
    } else {
        alert("The copied settings are not from the same type!")
    }
    document.querySelector("aside").setAttribute("data-setting-paste-active", "false");
}

// settings.cbxLayout

function decompileCbxLayout() {
    roleList.forEach(role => {
        if (settings.cbxLayout[`${role}`].active === true) {
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                if (settings.cbxLayout[`${role}`].value.type === "column" || settings.cbxLayout[`${role}`].value.type === "column-reverse") {
                    cbxW.querySelector(".usn").style.setProperty("--usnSizingH", settings.usnPosition[`${role}`].value.sizingH);
                    cbxW.querySelector(".usn").style.setProperty("--usnSizingV", settings.usnPosition[`${role}`].value.sizingV);
                    cbxW.querySelector(".msg").style.setProperty("--msgSizingH", settings.msgPosition[`${role}`].value.sizingH);
                    cbxW.querySelector(".msg").style.setProperty("--msgSizingV", settings.msgPosition[`${role}`].value.sizingV);
                } else if (settings.cbxLayout[`${role}`].value.type === "row" || settings.cbxLayout[`${role}`].value.type === "row-reverse") {
                    cbxW.querySelector(".usn").style.setProperty("--usnSizingH", settings.usnPosition[`${role}`].value.sizingV);
                    cbxW.querySelector(".usn").style.setProperty("--usnSizingV", settings.usnPosition[`${role}`].value.sizingH);
                    cbxW.querySelector(".msg").style.setProperty("--msgSizingH", settings.msgPosition[`${role}`].value.sizingV);
                    cbxW.querySelector(".msg").style.setProperty("--msgSizingV", settings.msgPosition[`${role}`].value.sizingH);
                }
...
function updateLayout(type) {
    ...
    if (type === "main") {
        ...
        switch (index) {
            case 0:
                settings.cbxLayout[`${activeRole}`].value.type = "column";
                document.querySelector(".layoutRowOptions").setAttribute("data-active", "false");
            break;
            
            case 1:
                settings.cbxLayout[`${activeRole}`].value.type = "column-reverse";
                document.querySelector(".layoutRowOptions").setAttribute("data-active", "false");
            break;
            
            case 2:
                settings.cbxLayout[`${activeRole}`].value.type = "row";
                document.querySelector(".layoutRowOptions").setAttribute("data-active", "true");
            break;
                
            case 3:
                settings.cbxLayout[`${activeRole}`].value.type = "row-reverse";
                document.querySelector(".layoutRowOptions").setAttribute("data-active", "true");
            break;
        }
    } else if (type === "row") {
        document.querySelectorAll(".layoutHBtn").forEach(btn => {
            btn.setAttribute("data-active", "false");
        });
        ...
        switch (index) {
            case 0:
                settings.cbxLayout[`${activeRole}`].value.layoutRowSizing = "username";
            break;

            case 1:
                settings.cbxLayout[`${activeRole}`].value.layoutRowSizing = "space";
            break;

            case 2:
                settings.cbxLayout[`${activeRole}`].value.layoutRowSizing = "message";
            break;
        }
    }
...
function switchCbxMaskBG(type) {
    if (type === "BG") {
        ...
        event.target.parentNode.parentNode.querySelector(".backgroundCont").setAttribute("data-active", "true");
        event.target.parentNode.parentNode.querySelector(".maskCont").setAttribute("data-active", "false");
    } else if (type === "Mask") {
        ...
        event.target.parentNode.parentNode.querySelector(".backgroundCont").setAttribute("data-active", "false");
        event.target.parentNode.parentNode.querySelector(".maskCont").setAttribute("data-active", "true");
    }
}
...
function openColorCbxBGBox(grdNum, clrNum) {
    ...
    document.querySelectorAll(".colorMenu").forEach(menu => {
        menu.setAttribute("data-color-show", "false");
        menu.setAttribute("data-set-show", "false");
        menu.setAttribute("data-size-show", "false");
    });

    event.target.parentNode.setAttribute("data-color-show", open);
    ...
    document.querySelector(".colorMenuColCbxBG").querySelectorAll(".colorMenu")[activeCbxBGGrad].setAttribute("data-set-show", "false");
    document.querySelector(".colorMenuColCbxBG").querySelectorAll(".colorMenu")[activeCbxBGGrad].setAttribute("data-size-show", "false");
    ...
    document.querySelector(".colorMenuColCbxBG").querySelectorAll(".colorMenu")[activeCbxBGGrad].querySelector(".colorChangeOpacity").setAttribute("data-value", settings.cbxBackground[`${activeRole}`].value[activeCbxBGGrad].colors[activeCbxBGColor].opacity);
    document.querySelectorAll(".colorSliderCbxBG").forEach(cbxCont => {
        cbxCont.setAttribute("data-active", "false");
    });
    document.querySelectorAll(".colorSliderCbxBGCont")[activeCbxBGGrad].querySelectorAll(".colorSliderCbxBG")[activeCbxBGColor].setAttribute("data-active", "true");
}

function updateNameCbxBGBox() {
    for (var i = 0; i < document.querySelectorAll(".backgroundNameCbxBG").length; i++) {
        settings.cbxBackground[`${activeRole}`].value[i].name = document.querySelectorAll(".backgroundNameCbxBG")[i].value;
    }
}

function updateColorCbxBGBox(type) {
    if (event.target.classList.contains("colorChangeOpacity")) {
...
function hideColorCbxBGBox() {
    document.querySelectorAll(".colorSliderCbxBGCont")[activeCbxBGGrad].querySelectorAll(".colorSliderCbxBG")[activeCbxBGColor].setAttribute("data-active", "false");
    activeCbxBGGrad = undefined;
    activeCbxBGColor = undefined;
    event.target.parentNode.parentNode.setAttribute("data-color-show", "false");
}

function switchSetCbxBGBox() {
    var open = true;

    if (event.target.parentNode.getAttribute("data-set-show") === "true") {
        open = false;
    }
    ...
    event.target.parentNode.setAttribute("data-set-show", open);
}

function switchSizeCbxBGBox() {
    var open = true;

    if (event.target.parentNode.getAttribute("data-size-show") === "true") {
        open = false;
    }
    ...
    event.target.parentNode.setAttribute("data-size-show", open);
}

function addGradientCbxBG(type, index) {
    var indexPlace;
    if (index === undefined) {
        indexPlace = 0;
        if (type === "solid") {
            settings.cbxBackground[`${activeRole}`].value.splice(indexPlace, 0, {
                type: "solid",
                name: `Background ${settings.cbxBackground[`${activeRole}`].value.length + 1}`,
                colors: [
                    {
                        position:0,
                        type: "custom",
            ...
            var colorMenu = document.createElement("div");
                ...
                colorMenu.innerHTML = `
                    <div class="colorSliderAddRow">
                        <button class="colorSliderMoveUp icon" onclick="moveColorSliderCbxBG(event, 'up')" title="Moves this background one layer to the top"><img class="iconSVG" src="Icons/Diverse/UpLite.svg"></button>
                        <button class="colorSliderMoveDown icon" onclick="moveColorSliderCbxBG(event, 'down')" title="Moves this background one layer to the bottom"><img class="iconSVG" src="Icons/Diverse/DownLite.svg"></button>
                        <button class="colorSliderDuplicate icon" onclick="duplicateColorSliderCbxBG()" title="Duplicates this background to a new layer"><img class="iconSVG" src="Icons/Diverse/Copy.svg"></button>
                        <input class="backgroundNameCbxBG backgroundName" type="text" value="${settings.cbxBackground[`${activeRole}`].value[0].name}" oninput="updateNameCbxBGBox()">
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderCbxBGCont" style="background-color:#ffffff;">
                                <input class="colorSlider colorSliderCbxBG" oninput="updateColorSliderCbxBGBox()" onpointerdown="openColorCbxBGBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                        ...
                        <div class="colorChangeType selectItemRow">
                            <button class="icon" onclick="updateColorCbxBGBox('custom')"><input type="color" class="colorChangeInput" oninput="updateColorCbxBGBox()"><img class="iconSVG descrIconSVG" src="Icons/Diverse/Color.svg"></button>
                            <button class="icon" onclick="updateColorCbxBGBox('twitch')" title="Uses the color the user uses in the Twitch chat"><img class="iconSVG descrIconSVG" src="Icons/Diverse/TwitchColor.svg"></button>
                            <button class="icon colorDeleteBtn" onclick="deleteColorCbxBGBox()" title="Removes the selected color"><img class="iconSVG descrIconSVG" src="Icons/Diverse/Remove.svg"></button>
                        </div>
                        <input type="range" class="colorChangeOpacity" value="0" min="0" max="255" step="1" oninput="updateColorCbxBGBox()" data-value="255" title="Changes the opacity of the selected color">
                        <div class="presetCont">
                            <div class="presetList" data-delete-show="false">${insertPresetColor()}</div>
                    ...
                    <div class="bgSizeSettings">
                        <div class="numCont">
                            ...
                            <input class="numContInput" type="number" value="50" oninput="updateCbxBGSizePos('posX', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateCbxBGSizePos('posX', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">y-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateCbxBGSizePos('posY', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateCbxBGSizePos('posY', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">width:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateCbxBGSizePos('width', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateCbxBGSizePos('width', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">height:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateCbxBGSizePos('height', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateCbxBGSizePos('height', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                    </div>
                `;
            ...
            var colorMenu = document.createElement("div");
                ...
                colorMenu.innerHTML = `
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderCbxBGCont" style="background-image: linear-gradient(90deg, #000000 0%, #ffffff 100%);">
                                <input class="colorSlider colorSliderCbxBG" oninput="updateColorSliderCbxBGBox()" onpointerdown="openColorCbxBGBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                                <input class="colorSlider colorSliderCbxBG" oninput="updateColorSliderCbxBGBox()" onpointerdown="openColorCbxBGBox(0, 1)" type="range" value="100" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                    ...
                    <button class="showGradSpecBtn" onclick="switchSetCbxBGBox()" data-active="false" title="Opens the settings specific to this type of gradient"><img class="iconSVG" src="Icons/Diverse/Settings.svg"></button>
        ...
        document.querySelector(".colorMenuColCbxBG").insertBefore(colorMenu, document.querySelector(".colorMenuColCbxBG").children[indexPlace]);
    } else {
        indexPlace = index;

        var menuOpt = JSON.parse(JSON.stringify(settings.cbxBackground[`${activeRole}`].value[indexPlace]));
        settings.cbxBackground[`${activeRole}`].value.splice(indexPlace, 0, menuOpt);

        var colorMenuNode = document.querySelector(".colorMenuColCbxBG").querySelectorAll(".colorMenu")[indexPlace].cloneNode(true);
        colorMenuNode.setAttribute("data-color-show", "false");
        colorMenuNode.setAttribute("data-set-show", "false");
        colorMenuNode.setAttribute("data-size-show", "false");
        document.querySelector(".colorMenuColCbxBG").insertBefore(colorMenuNode, document.querySelector(".colorMenuColCbxBG").children[indexPlace]);
    }
    
    for (var i = 0; i < document.querySelectorAll(".colorSliderCbxBGCont").length; i++) {
        for (var j = 0; j < document.querySelectorAll(".colorSliderCbxBGCont")[i].querySelectorAll(".colorSlider").length; j++) {
            document.querySelectorAll(".colorSliderCbxBGCont")[i].querySelectorAll(".colorSlider")[j].setAttribute("onpointerdown", `openColorCbxBGBox(${i}, ${j})`);
        }
    }
...
function updateRadialPosCbxBG(e, xy) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode.parentNode);
    if (xy === "x") {
        settings.cbxBackground[`${activeRole}`].value[i].x = parseInt(e.target.value);
    } else if (xy === "y") {
        settings.cbxBackground[`${activeRole}`].value[i].y = parseInt(e.target.value);
    }
...
function duplicateColorSliderCbxBG() {
    activeCbxBGGrad += 1;
    var i = Array.from(event.target.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode);
    var type = settings.cbxBackground[`${activeRole}`].value[i].type;

    addGradientCbxBG(type, i);
}

function updateCbxBGSizePos(cat, value) {
    var i = Array.from(event.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode.parentNode);
    if (cat === "posX") {
        if (value === "%") {
            settings.cbxBackground[`${activeRole}`].value[i].posX.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.cbxBackground[`${activeRole}`].value[i].posX.px = parseInt(event.target.value);
        }
    } else if (cat === "posY") {
        if (value === "%") {
            settings.cbxBackground[`${activeRole}`].value[i].posY.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.cbxBackground[`${activeRole}`].value[i].posY.px = parseInt(event.target.value);
        }
    } else if (cat === "width") {
        if (value === "%") {
            settings.cbxBackground[`${activeRole}`].value[i].width.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.cbxBackground[`${activeRole}`].value[i].width.px = parseInt(event.target.value);
        }
    } else if (cat === "height") {
        if (value === "%") {
            settings.cbxBackground[`${activeRole}`].value[i].height.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.cbxBackground[`${activeRole}`].value[i].height.px = parseInt(event.target.value);
        }
    }
...
// settings.cbxBackgroundMask

var activeCbxMaskGrad = 0;
var activeCbxMaskColor = 0;

function decompileCbxMask() {
    roleList.forEach(role => {
...
function updateColorSliderCbxMaskBox() {
    for (let i = 0; i < document.querySelectorAll(".colorSliderCbxMaskCont").length; i++) {
        if (settings.cbxBackgroundMask[`${activeRole}`].value[i].type !== "solid") {
            
            var cbxMaskPrev = "linear-gradient(90deg, ",
                nodeList = document.querySelectorAll(".colorSliderCbxMaskCont")[i].querySelectorAll(".colorSliderCbxMask"),
                colorList = settings.cbxBackgroundMask[`${activeRole}`].value[i].colors;

            for (let j = 0; j < settings.cbxBackgroundMask[`${activeRole}`].value[i].colors.length; j++) {
                settings.cbxBackgroundMask[`${activeRole}`].value[i].colors[j].count = 0;
                colorList[j].position = parseFloat(nodeList[j].value);
                nodeList[j].style.setProperty("--value", `${nodeList[j].value}%`);
                nodeList[j].style.setProperty("--valueTxt", `"${nodeList[j].value}"`);
            }
            ...
            for (let j = 0; j < settings.cbxBackgroundMask[`${activeRole}`].value[i].colors.length; j++) {
                for (let k = 0; k < settings.cbxBackgroundMask[`${activeRole}`].value[i].colors.length; k++) {
                    if (k !== j) {
                        if (settings.cbxBackgroundMask[`${activeRole}`].value[i].colors[j].position >= settings.cbxBackgroundMask[`${activeRole}`].value[i].colors[k].position) {
                            settings.cbxBackgroundMask[`${activeRole}`].value[i].colors[j].count += 1;
                        }
                    }
                }
            }
            ...
            for (let j = 0; j < settings.cbxBackgroundMask[`${activeRole}`].value[i].colors.length; j++) {
                var resultElmnt = settings.cbxBackgroundMask[`${activeRole}`].value[i].colors.find(findElmnt => findElmnt.count === j);
                cbxMaskPrev += `#ffffff${dec2hex(resultElmnt.opacity)} ${resultElmnt.position}%`;
                
                if (j < settings.cbxBackgroundMask[`${activeRole}`].value[i].colors.length - 1) {
                    cbxMaskPrev += ", ";
                } else {
                    cbxMaskPrev += ")";
                }
            ...
            document.querySelectorAll(".colorSliderCbxMaskCont")[i].style.backgroundImage = cbxMaskPrev;
        } else {
            var cbxMaskPrev = `#ffffff${dec2hex(settings.cbxBackgroundMask[`${activeRole}`].value[i].colors[0].opacity)}`;
            document.querySelectorAll(".colorSliderCbxMaskCont")[i].style.backgroundColor = cbxMaskPrev;
        }
    }
    decompileCbxMask();
}

function openColorCbxMaskBox(grdNum, clrNum) {
    var open = true;
    ...
    activeCbxMaskGrad = grdNum;
    activeCbxMaskColor = clrNum;
    document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].setAttribute("data-color-show", "true");
    document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].setAttribute("data-set-show", "false");
    document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].setAttribute("data-size-show", "false");
    document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].querySelector(".colorChangeOpacity").value = settings.cbxBackgroundMask[`${activeRole}`].value[activeCbxMaskGrad].colors[activeCbxMaskColor].opacity;
    document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].querySelector(".colorChangeOpacity").setAttribute("data-value", settings.cbxBackgroundMask[`${activeRole}`].value[activeCbxMaskGrad].colors[activeCbxMaskColor].opacity);
    document.querySelectorAll(".colorSliderCbxMask").forEach(cbxCont => {
        cbxCont.setAttribute("data-active", "false");
    });
    document.querySelectorAll(".colorSliderCbxMaskCont")[activeCbxMaskGrad].querySelectorAll(".colorSliderCbxMask")[activeCbxMaskColor].setAttribute("data-active", "true");
}

function updateNameCbxMaskBox() {
    for (var i = 0; i < document.querySelectorAll(".backgroundNameCbxMask").length; i++) {
        settings.cbxBackgroundMask[`${activeRole}`].value[i].name = document.querySelectorAll(".backgroundNameCbxMask")[i].value;
    }
}

function updateColorCbxMaskBox(type) {
    if (event.target.classList.contains("colorChangeOpacity")) {
        event.target.setAttribute("data-value", event.target.value);
    }
    
    settings.cbxBackgroundMask[`${activeRole}`].value[activeCbxMaskGrad].colors[activeCbxMaskColor].opacity = parseInt(document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].querySelector(".colorChangeOpacity").value);
    updateColorSliderCbxMaskBox();

    decompileCbxMask();
}

function deleteColorCbxMaskBox() {
    settings.cbxBackgroundMask[`${activeRole}`].value[activeCbxMaskGrad].colors.splice(activeCbxMaskColor, 1);
    document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].querySelectorAll(".colorSliderCbxMask")[activeCbxMaskColor].remove();
    updateColorSliderCbxMaskBox();
    for (let i = 0; i < document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].querySelectorAll(".colorSliderCbxMask").length; i++) {
        document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].querySelectorAll(".colorSliderCbxMask")[i].setAttribute("onpointerdown", `openColorCbxMaskBox(${activeCbxMaskGrad}, ${i})`);
    }
    document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[activeCbxMaskGrad].setAttribute("data-color-show", "false");
    activeCbxMaskGrad = undefined;
    activeCbxMaskColor = undefined;

    for (let j = 0; j < settings.cbxBackgroundMask[`${activeRole}`].value.length; j++) {
        for (let k = 0; k < settings.cbxBackgroundMask[`${activeRole}`].value[j].colors.length; k++) {
            document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[j].querySelectorAll(".colorSlider")[k].setAttribute("onpointerdown", `openColorCbxMaskBox(${j}, ${k})`)
        }
    }
...
function addColorSliderCbxMask(e) {
    i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode);
    document.querySelectorAll(".colorSlider").forEach(slider => {
        slider.setAttribute("data-active", "false");
    })
    var newSlider = document.createElement("input");newSlider.className = "colorSlider colorSliderCbxMask";newSlider.setAttribute("oninput", "updateColorSliderCbxMaskBox()");newSlider.setAttribute("onpointerdown", `openColorCbxMaskBox(${i}, ${e.target.parentNode.querySelectorAll(".colorSlider").length})`);newSlider.type = "range";newSlider.value = (e.layerX / e.target.clientWidth) * 100;newSlider.min = "0";newSlider.max = "100";newSlider.step = "0.1";newSlider.setAttribute("data-active", "true");
                
    document.querySelectorAll(".colorSliderCbxMaskCont")[i].appendChild(newSlider);

    settings.cbxBackgroundMask[`${activeRole}`].value[i].colors.push({
        position: newSlider.value, // In %
        opacity: 255, // In Hex
        count: 0
    });
    updateColorSliderCbxMaskBox();
    openColorCbxMaskBox(i, e.target.parentNode.querySelectorAll(".colorSlider").length - 1);
...
function hideColorCbxMaskBox() {
    document.querySelectorAll(".colorSliderCbxMaskCont")[activeCbxMaskGrad].querySelectorAll(".colorSliderCbxMask")[activeCbxMaskColor].setAttribute("data-active", "false");
    activeCbxMaskGrad = undefined;
    activeCbxMaskColor = undefined;
    event.target.parentNode.parentNode.setAttribute("data-color-show", "false");
}

function switchSetCbxMaskBox() {
    var open = true;
...
function addGradientCbxMask(type, index) {
    var indexPlace;
    if (index === undefined) {
        indexPlace = 0;
        if (type === "solid") {
            settings.cbxBackgroundMask[`${activeRole}`].value.splice(indexPlace, 0, {
                type: "solid",
                name: `Mask ${settings.cbxBackgroundMask[`${activeRole}`].value.length + 1}`,
                colors: [
                    {
                        position:0,
                        opacity:255,
            ...
            var colorMenu = document.createElement("div");
                ...
                colorMenu.innerHTML = `
                    <div class="colorSliderAddRow">
                        <button class="colorSliderMoveUp icon" onclick="moveColorSliderCbxMask(event, 'up')" title="Moves this background one layer to the top"><img class="iconSVG" src="Icons/Diverse/UpLite.svg"></button>
                        <button class="colorSliderMoveDown icon" onclick="moveColorSliderCbxMask(event, 'down')" title="Moves this background one layer to the bottom"><img class="iconSVG" src="Icons/Diverse/DownLite.svg"></button>
                        <button class="colorSliderDuplicate icon" onclick="duplicateColorSliderCbxMask()" title="Duplicates this background to a new layer"><img class="iconSVG" src="Icons/Diverse/Copy.svg"></button>
                        <input class="backgroundNameCbxMask backgroundName" type="text" value="${settings.cbxBackgroundMask[`${activeRole}`].value[0].name}" oninput="updateNameCbxMaskBox()">
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderCbxMaskCont" style="background-color:#ffffff;">
                                <input class="colorSlider colorSliderCbxMask" oninput="updateColorSliderCbxMaskBox()" onpointerdown="openColorCbxMaskBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                        ...
                        <input type="range" class="colorChangeOpacity" value="0" min="0" max="255" step="1" oninput="updateColorCbxMaskBox()" data-value="255" title="Changes the opacity of the selected color">
                        <button class="icon colorDeleteBtn" onclick="deleteColorCbxMaskBox()" title="Removes the selected color"><img class="iconSVG descrIconSVG" src="Icons/Diverse/Remove.svg"></button>
                    </div>
                    ...
                    <div class="bgSizeSettings">
                        <div class="numCont">
                            <span class="numContDescr">x-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateCbxMaskSizePos('posX', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateCbxMaskSizePos('posX', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">y-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateCbxMaskSizePos('posY', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateCbxMaskSizePos('posY', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">width:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateCbxMaskSizePos('width', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateCbxMaskSizePos('width', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">height:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateCbxMaskSizePos('height', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateCbxMaskSizePos('height', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                    </div>
                `;
                ...
                colorMenu.innerHTML = `
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderCbxMaskCont" style="background-image: linear-gradient(90deg, #000000 0%, #ffffff 100%);">
                                <input class="colorSlider colorSliderCbxMask" oninput="updateColorSliderCbxMaskBox()" onpointerdown="openColorCbxMaskBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                                <input class="colorSlider colorSliderCbxMask" oninput="updateColorSliderCbxMaskBox()" onpointerdown="openColorCbxMaskBox(0, 1)" type="range" value="100" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                    ...
                    <button class="showGradSpecBtn" onclick="switchSetCbxMaskBox()" data-active="false" title="Opens the settings specific to this type of gradient"><img class="iconSVG" src="Icons/Diverse/Settings.svg"></button>
                    <div class="gradSpecSettings">
                        <div class="inputNumRow">
                            <div class="numCont">
                                <span class="numContDescr">rotation:</span>
                                <input class="numContInput" type="number" value="90" oninput="updateLinearRotCbxMask(event)">
                                <span class="numContUnit">°</span>
                            </div>
                        </div>
                    </div>
                ...
                colorMenu.innerHTML = `
                    ...
                    <div class="gradSpecSettings">
                        <div class="singleChoiceList gradRadialPosContCbxMask">
                            <button data-active="true" onclick="updateRadialTypeCbxMask(event, 'farthest-corner')">Farthest Corner</button>
                            <button data-active="false" onclick="updateRadialTypeCbxMask(event, 'farthest-side')">Farthest Edge</button>
                            <button data-active="false" onclick="updateRadialTypeCbxMask(event, 'closest-corner')">Closest Corner</button>
                            <button data-active="false" onclick="updateRadialTypeCbxMask(event, 'closest-side')">Closest Edge</button>
                            <button data-active="false" onclick="updateRadialTypeCbxMask(event, 'circle')">Circle</button>
                        </div>
                        <div class="inputNumRow">
                            <div class="numCont">
                                <span class="numContDescr">x:</span>
                                <input class="numContInput" type="number" value="50" oninput="updateRadialPosCbxMask(event, 'x')">
                                <span class="numContUnit">%</span>
                            </div>
                            <div class="numCont">
                                <span class="numContDescr">y:</span>
                                <input class="numContInput" type="number" value="50" oninput="updateRadialPosCbxMask(event, 'y')">
                                <span class="numContUnit">%</span>
                            </div>
                        </div>
                    </div>
        ...
        document.querySelector(".colorMenuColCbxMask").insertBefore(colorMenu, document.querySelector(".colorMenuColCbxMask").children[indexPlace]);
    } else {
        indexPlace = index;

        var menuOpt = JSON.parse(JSON.stringify(settings.cbxBackgroundMask[`${activeRole}`].value[indexPlace]));
        settings.cbxBackgroundMask[`${activeRole}`].value.splice(indexPlace, 0, menuOpt);

        var colorMenuNode = document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[indexPlace].cloneNode(true);
        colorMenuNode.setAttribute("data-color-show", "false");
        colorMenuNode.setAttribute("data-set-show", "false");
        colorMenuNode.setAttribute("data-size-show", "false");
        document.querySelector(".colorMenuColCbxMask").insertBefore(colorMenuNode, document.querySelector(".colorMenuColCbxMask").children[indexPlace]);
    }
    
    for (var i = 0; i < document.querySelectorAll(".colorSliderCbxMaskCont").length; i++) {
        for (var j = 0; j < document.querySelectorAll(".colorSliderCbxMaskCont")[i].querySelectorAll(".colorSlider").length; j++) {
            document.querySelectorAll(".colorSliderCbxMaskCont")[i].querySelectorAll(".colorSlider")[j].setAttribute("onpointerdown", `openColorCbxMaskBox(${i}, ${j})`);
        }
    }
...
function removeGradientCbxMask(e) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode);
    if (i < activeCbxMaskGrad) {
        activeCbxMaskGrad -= 1;
    }
    settings.cbxBackgroundMask[`${activeRole}`].value.splice(i, 1);
    document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[i].remove();
    for (let j = 0; j < document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu").length; j++) {
        for (let k = 0; k < document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[j].querySelectorAll(".colorSlider").length; k++) {
            document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[j].querySelectorAll(".colorSlider")[k].setAttribute("onpointerdown", `openColorCbxMaskBox(${j}, ${k})`)
        }
    }
...
function updateRadialTypeCbxMask(e, size) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode);
    settings.cbxBackgroundMask[`${activeRole}`].value[i].size = size;
    for (let j = 0; j < settings.cbxBackgroundMask[`${activeRole}`].value.length; j++) {
        if (settings.cbxBackgroundMask[`${activeRole}`].value[j].type === "radial") {
            for (let k = 0; k < document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[j].querySelector(".gradRadialPosContCbxMask").querySelectorAll("button").length; k++) {
                document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[j].querySelector(".gradRadialPosContCbxMask").querySelectorAll("button")[k].setAttribute("data-active", "false");
            }
            e.target.setAttribute("data-active", "true");
        }
    }
...
function updateLinearRotCbxMask(e) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode.parentNode);
    settings.cbxBackgroundMask[`${activeRole}`].value[i].rotate = parseInt(e.target.value);
    
    decompileCbxMask();
}

function updateRadialPosCbxMask(e, xy) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode.parentNode);
    if (xy === "x") {
        settings.cbxBackgroundMask[`${activeRole}`].value[i].x = parseInt(e.target.value);
    } else if (xy === "y") {
        settings.cbxBackgroundMask[`${activeRole}`].value[i].y = parseInt(e.target.value);
    }
    
    decompileCbxMask();
}

function moveColorSliderCbxMask(e, dir) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode);
    if (dir === "up" && i > 0) {
        var elmnt = settings.cbxBackgroundMask[`${activeRole}`].value[i];
        settings.cbxBackgroundMask[`${activeRole}`].value.splice(i, 1);
        settings.cbxBackgroundMask[`${activeRole}`].value.splice(i - 1, 0, elmnt);
        document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[i - 1].before(document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[i]);
        if (activeCbxMaskGrad === i) {
            activeCbxMaskGrad -= 1;
        } else if (activeCbxMaskGrad === i - 1) {
            activeCbxMaskGrad += 1;
        }
    } else if (dir === "down" && i < settings.cbxBackgroundMask[`${activeRole}`].value.length - 1) {
        var elmnt = settings.cbxBackgroundMask[`${activeRole}`].value[i];
        settings.cbxBackgroundMask[`${activeRole}`].value.splice(i, 1);
        settings.cbxBackgroundMask[`${activeRole}`].value.splice(i + 1, 0, elmnt);
        document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[i + 1].after(document.querySelector(".colorMenuColCbxMask").querySelectorAll(".colorMenu")[i]);
        if (activeCbxMaskGrad === i) {
            activeCbxMaskGrad += 1;
        } else if (activeCbxMaskGrad === i + 1) {
            activeCbxMaskGrad -= 1;
...
function duplicateColorSliderCbxMask() {
    activeCbxMaskGrad += 1;
    var i = Array.from(event.target.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode);
    var type = settings.cbxBackgroundMask[`${activeRole}`].value[i].type;

    addGradientCbxMask(type, i);
}

function updateCbxMaskSizePos(cat, value) {
    var i = Array.from(event.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode.parentNode);
    if (cat === "posX") {
        if (value === "%") {
            settings.cbxBackgroundMask[`${activeRole}`].value[i].posX.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.cbxBackgroundMask[`${activeRole}`].value[i].posX.px = parseInt(event.target.value);
        }
    } else if (cat === "posY") {
        if (value === "%") {
            settings.cbxBackgroundMask[`${activeRole}`].value[i].posY.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.cbxBackgroundMask[`${activeRole}`].value[i].posY.px = parseInt(event.target.value);
        }
    } else if (cat === "width") {
        if (value === "%") {
            settings.cbxBackgroundMask[`${activeRole}`].value[i].width.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.cbxBackgroundMask[`${activeRole}`].value[i].width.px = parseInt(event.target.value);
        }
    } else if (cat === "height") {
        if (value === "%") {
            settings.cbxBackgroundMask[`${activeRole}`].value[i].height.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.cbxBackgroundMask[`${activeRole}`].value[i].height.px = parseInt(event.target.value);
        }
    }
...
function openBorderMenuCbx(pos) {
    ...
    if (activeCbxBorder === "top" || activeCbxBorder === "bottom" || activeCbxBorder === "left" || activeCbxBorder === "right") {
        ...
        borderMenu.querySelector(".borderColorMenu").querySelector(".colorChangeOpacity").setAttribute("data-value", borderMenu.querySelector(".borderColorMenu").querySelector(".colorChangeOpacity").value);
...
function openShadowCbx(triggerAdd) {
    ...
    if (activeCbxShadow >= 0) {
        ...
        document.querySelector(".shadowSettingsCbx").querySelectorAll(".numContInput")[0].value = settings.cbxShadow[`${activeRole}`].value[activeCbxShadow].x;
        document.querySelector(".shadowSettingsCbx").querySelectorAll(".numContInput")[1].value = settings.cbxShadow[`${activeRole}`].value[activeCbxShadow].y;
        ...
        document.querySelector("#cbxShadowOpacity").setAttribute("data-value", settings.cbxShadow[`${activeRole}`].value[activeCbxShadow].color.opacity);
...
function updateShadowCbx(type, value) {
    ...
    settings.cbxShadow[`${activeRole}`].value[activeCbxShadow].x    = parseInt(document.querySelector(".shadowSettingsCbx").querySelectorAll(".numContInput")[0].value);
    settings.cbxShadow[`${activeRole}`].value[activeCbxShadow].y    = parseInt(document.querySelector(".shadowSettingsCbx").querySelectorAll(".numContInput")[1].value);
    ...
    document.querySelector("#cbxShadowOpacity").setAttribute("data-value", document.querySelector("#cbxShadowOpacity").value);
    ...
    for (let i = 0; i < settings.cbxShadow[`${activeRole}`].value.length; i++) {
        var cbxStyle = `${settings.cbxShadow[`${activeRole}`].value[i].x}px ${settings.cbxShadow[`${activeRole}`].value[i].y}px ${settings.cbxShadow[`${activeRole}`].value[i].blur}px ${settings.cbxShadow[`${activeRole}`].value[i].expand}px ${settings.cbxShadow[`${activeRole}`].value[i].color.color}${dec2hex(settings.cbxShadow[`${activeRole}`].value[i].color.opacity)}`;
...
function decompileUsnBG() {
    roleList.forEach(role => {
        if (settings.usnBackground[`${role}`].active === true) {
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                var usn = cbxW.querySelector(".usn");
                var usnBackground = "",
                    usnBackgroundPos = "",
                    usnBackgroundSize = "";
                for (let i = 0; i < settings.usnBackground[`${role}`].value.length; i++) {
                    if (settings.usnBackground[`${role}`].value[i].type === "solid") {
                        if (settings.usnBackground[`${role}`].value[i].colors[0].type === "custom") {
                            ...
                            usnBackground += `linear-gradient(${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.usnBackground[`${role}`].value[i].colors[0].opacity)}, ${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.usnBackground[`${role}`].value[i].colors[0].opacity)})`;
                        }
                        ...
                        for (let j = 0; j < sortedColors.length; j++) {
                            if (sortedColors[j].type === "custom") {
                                ...
                                usnBackground += `${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(sortedColors[j].opacity)} ${sortedColors[j].position}%`
                            }
...
function openColorUsnBGBox(grdNum, clrNum) {
    ...
    document.querySelector(".colorMenuColUsnBG").querySelectorAll(".colorMenu")[activeUsnBGGrad].setAttribute("data-set-show", "false");
    document.querySelector(".colorMenuColUsnBG").querySelectorAll(".colorMenu")[activeUsnBGGrad].setAttribute("data-size-show", "false");
    ...
    document.querySelector(".colorMenuColUsnBG").querySelectorAll(".colorMenu")[activeUsnBGGrad].querySelector(".colorChangeOpacity").setAttribute("data-value", settings.usnBackground[`${activeRole}`].value[activeUsnBGGrad].colors[activeUsnBGColor].opacity);
    document.querySelectorAll(".colorSliderUsnBG").forEach(usnCont => {
        usnCont.setAttribute("data-active", "false");
    });
    document.querySelectorAll(".colorSliderUsnBGCont")[activeUsnBGGrad].querySelectorAll(".colorSliderUsnBG")[activeUsnBGColor].setAttribute("data-active", "true");
}

function updateNameUsnBGBox() {
    for (var i = 0; i < document.querySelectorAll(".backgroundNameUsnBG").length; i++) {
        settings.usnBackground[`${activeRole}`].value[i].name = document.querySelectorAll(".backgroundNameUsnBG")[i].value;
    }
}
...
function hideColorUsnBGBox() {
    document.querySelectorAll(".colorSliderUsnBGCont")[activeUsnBGGrad].querySelectorAll(".colorSliderUsnBG")[activeUsnBGColor].setAttribute("data-active", "false");
    activeUsnBGGrad = undefined;
    activeUsnBGColor = undefined;
    event.target.parentNode.parentNode.setAttribute("data-color-show", "false");
}

function switchSetUsnBGBox() {
    var open = true;
...
function addGradientUsnBG(type, index) {
    var indexPlace;
    if (index === undefined) {
        indexPlace = 0;
        if (type === "solid") {
            settings.usnBackground[`${activeRole}`].value.splice(indexPlace, 0, {
                type: "solid",
                name: `Background ${settings.usnBackground[`${activeRole}`].value.length + 1}`,
                colors: [
                    {
                        position:0,
                        type: "custom",
            ...
            var colorMenu = document.createElement("div");
                ...
                colorMenu.innerHTML = `
                    <div class="colorSliderAddRow">
                        <button class="colorSliderMoveUp icon" onclick="moveColorSliderUsnBG(event, 'up')" title="Moves this background one layer to the top"><img class="iconSVG" src="Icons/Diverse/UpLite.svg"></button>
                        <button class="colorSliderMoveDown icon" onclick="moveColorSliderUsnBG(event, 'down')" title="Moves this background one layer to the bottom"><img class="iconSVG" src="Icons/Diverse/DownLite.svg"></button>
                        <button class="colorSliderDuplicate icon" onclick="duplicateColorSliderUsnBG()" title="Duplicates this background to a new layer"><img class="iconSVG" src="Icons/Diverse/Copy.svg"></button>
                        <input class="backgroundNameUsnBG backgroundName" type="text" value="${settings.usnBackground[`${activeRole}`].value[0].name}" oninput="updateNameUsnBGBox()">
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderUsnBGCont" style="background-color:#ffffff;">
                                <input class="colorSlider colorSliderUsnBG" oninput="updateColorSliderUsnBGBox()" onpointerdown="openColorUsnBGBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                        <button class="hideColorBtn" onclick="hideColorUsnBGBox()" title="Hides the color panel"><img class="iconSVG" src="Icons/Diverse/UpLite.svg"></button>
                        <div class="colorChangeType selectItemRow">
                            ...
                            <button class="icon" onclick="updateColorUsnBGBox('twitch')" title="Uses the color the user uses in the Twitch chat"><img class="iconSVG descrIconSVG" src="Icons/Diverse/TwitchColor.svg"></button>
                            <button class="icon colorDeleteBtn" onclick="deleteColorUsnBGBox()" title="Removes the selected color"><img class="iconSVG descrIconSVG" src="Icons/Diverse/Remove.svg"></button>
                        </div>
                        <input type="range" class="colorChangeOpacity" value="0" min="0" max="255" step="1" oninput="updateColorUsnBGBox()" data-value="255" title="Changes the opacity of the selected color">
                        <div class="presetCont">
                            <div class="presetList" data-delete-show="false">${insertPresetColor()}</div>
                            <button class="presetAdd" onclick="addPreset()"><img class="iconSVG" src="Icons/Diverse/Add.svg"></button>
                            <button class="presetShowDelete" onclick="showDeletePreset()"><img class="iconSVG" src="Icons/Diverse/Remove.svg"></button>
                    ...
                    <div class="bgSizeSettings">
                        <div class="numCont">
                            <span class="numContDescr">x-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateUsnBGSizePos('posX', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateUsnBGSizePos('posX', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">y-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateUsnBGSizePos('posY', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateUsnBGSizePos('posY', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">width:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateUsnBGSizePos('width', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateUsnBGSizePos('width', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">height:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateUsnBGSizePos('height', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateUsnBGSizePos('height', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                    </div>
                `;
            ...
            var colorMenu = document.createElement("div");
                ...
                colorMenu.innerHTML = `
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderUsnBGCont" style="background-image: linear-gradient(90deg, #000000 0%, #ffffff 100%);">
                                <input class="colorSlider colorSliderUsnBG" oninput="updateColorSliderUsnBGBox()" onpointerdown="openColorUsnBGBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                                <input class="colorSlider colorSliderUsnBG" oninput="updateColorSliderUsnBGBox()" onpointerdown="openColorUsnBGBox(0, 1)" type="range" value="100" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                    ...
                    <button class="showGradSpecBtn" onclick="switchSetUsnBGBox()" data-active="false" title="Opens the settings specific to this type of gradient"><img class="iconSVG" src="Icons/Diverse/Settings.svg"></button>
                    <div class="gradSpecSettings">
                        <div class="inputNumRow">
                            <div class="numCont">
                                <span class="numContDescr">rotation:</span>
        ...
        document.querySelector(".colorMenuColUsnBG").prepend(colorMenu);
    } else {
        indexPlace = index;

        var menuOpt = JSON.parse(JSON.stringify(settings.usnBackground[`${activeRole}`].value[indexPlace]));
        settings.usnBackground[`${activeRole}`].value.splice(indexPlace, 0, menuOpt);

        var colorMenuNode = document.querySelector(".colorMenuColUsnBG").querySelectorAll(".colorMenu")[indexPlace].cloneNode(true);
        colorMenuNode.setAttribute("data-color-show", "false");
        colorMenuNode.setAttribute("data-set-show", "false");
        colorMenuNode.setAttribute("data-size-show", "false");
        document.querySelector(".colorMenuColUsnBG").insertBefore(colorMenuNode, document.querySelector(".colorMenuColUsnBG").children[indexPlace]);
    }
    
    for (var i = 0; i < document.querySelectorAll(".colorSliderUsnBGCont").length; i++) {
        for (var j = 0; j < document.querySelectorAll(".colorSliderUsnBGCont")[i].querySelectorAll(".colorSlider").length; j++) {
            document.querySelectorAll(".colorSliderUsnBGCont")[i].querySelectorAll(".colorSlider")[j].setAttribute("onpointerdown", `openColorUsnBGBox(${i}, ${j})`);
        }
    }
...
function updateRadialPosUsnBG(e, xy) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode.parentNode);
    if (xy === "x") {
        settings.usnBackground[`${activeRole}`].value[i].x = parseInt(e.target.value);
    } else if (xy === "y") {
        settings.usnBackground[`${activeRole}`].value[i].y = parseInt(e.target.value);
    }
...
function duplicateColorSliderUsnBG() {
    activeUsnBGGrad += 1;
    var i = Array.from(event.target.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode);
    var type = settings.usnBackground[`${activeRole}`].value[i].type;

    addGradientUsnBG(type, i);
}

function updateUsnBGSizePos(cat, value) {
    var i = Array.from(event.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode.parentNode);
    if (cat === "posX") {
        if (value === "%") {
            settings.usnBackground[`${activeRole}`].value[i].posX.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.usnBackground[`${activeRole}`].value[i].posX.px = parseInt(event.target.value);
        }
    } else if (cat === "posY") {
        if (value === "%") {
            settings.usnBackground[`${activeRole}`].value[i].posY.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.usnBackground[`${activeRole}`].value[i].posY.px = parseInt(event.target.value);
        }
    } else if (cat === "width") {
        if (value === "%") {
            settings.usnBackground[`${activeRole}`].value[i].width.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.usnBackground[`${activeRole}`].value[i].width.px = parseInt(event.target.value);
        }
    } else if (cat === "height") {
        if (value === "%") {
            settings.usnBackground[`${activeRole}`].value[i].height.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.usnBackground[`${activeRole}`].value[i].height.px = parseInt(event.target.value);
        }
    }
...
// settings.usnBackgroundMask

var activeUsnMaskGrad = 0;
var activeUsnMaskColor = 0;

function decompileUsnMask() {
    roleList.forEach(role => {
...
function updateColorSliderUsnMaskBox() {
    for (let i = 0; i < document.querySelectorAll(".colorSliderUsnMaskCont").length; i++) {
        if (settings.usnBackgroundMask[`${activeRole}`].value[i].type !== "solid") {
            
            var usnMaskPrev = "linear-gradient(90deg, ",
                nodeList = document.querySelectorAll(".colorSliderUsnMaskCont")[i].querySelectorAll(".colorSliderUsnMask"),
                colorList = settings.usnBackgroundMask[`${activeRole}`].value[i].colors;

            for (let j = 0; j < settings.usnBackgroundMask[`${activeRole}`].value[i].colors.length; j++) {
                settings.usnBackgroundMask[`${activeRole}`].value[i].colors[j].count = 0;
                colorList[j].position = parseFloat(nodeList[j].value);
                nodeList[j].style.setProperty("--value", `${nodeList[j].value}%`);
                nodeList[j].style.setProperty("--valueTxt", `"${nodeList[j].value}"`);
            }
            ...
            for (let j = 0; j < settings.usnBackgroundMask[`${activeRole}`].value[i].colors.length; j++) {
                for (let k = 0; k < settings.usnBackgroundMask[`${activeRole}`].value[i].colors.length; k++) {
                    if (k !== j) {
                        if (settings.usnBackgroundMask[`${activeRole}`].value[i].colors[j].position >= settings.usnBackgroundMask[`${activeRole}`].value[i].colors[k].position) {
                            settings.usnBackgroundMask[`${activeRole}`].value[i].colors[j].count += 1;
                        }
                    }
                }
            }
            ...
            for (let j = 0; j < settings.usnBackgroundMask[`${activeRole}`].value[i].colors.length; j++) {
                var resultElmnt = settings.usnBackgroundMask[`${activeRole}`].value[i].colors.find(findElmnt => findElmnt.count === j);
                usnMaskPrev += `#ffffff${dec2hex(resultElmnt.opacity)} ${resultElmnt.position}%`;
                
                if (j < settings.usnBackgroundMask[`${activeRole}`].value[i].colors.length - 1) {
                    usnMaskPrev += ", ";
                } else {
                    usnMaskPrev += ")";
                }
            ...
            document.querySelectorAll(".colorSliderUsnMaskCont")[i].style.backgroundImage = usnMaskPrev;
        } else {
            var usnMaskPrev = `#ffffff${dec2hex(settings.usnBackgroundMask[`${activeRole}`].value[i].colors[0].opacity)}`;
            document.querySelectorAll(".colorSliderUsnMaskCont")[i].style.backgroundColor = usnMaskPrev;
        }
    }
    decompileUsnMask();
}

function openColorUsnMaskBox(grdNum, clrNum) {
    var open = true;
    ...
    activeUsnMaskGrad = grdNum;
    activeUsnMaskColor = clrNum;
    document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].setAttribute("data-color-show", "true");
    document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].setAttribute("data-set-show", "false");
    document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].setAttribute("data-size-show", "false");
    document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].querySelector(".colorChangeOpacity").value = settings.usnBackgroundMask[`${activeRole}`].value[activeUsnMaskGrad].colors[activeUsnMaskColor].opacity;
    document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].querySelector(".colorChangeOpacity").setAttribute("data-value", settings.usnBackgroundMask[`${activeRole}`].value[activeUsnMaskGrad].colors[activeUsnMaskColor].opacity);
    document.querySelectorAll(".colorSliderUsnMask").forEach(usnCont => {
        usnCont.setAttribute("data-active", "false");
    });
    document.querySelectorAll(".colorSliderUsnMaskCont")[activeUsnMaskGrad].querySelectorAll(".colorSliderUsnMask")[activeUsnMaskColor].setAttribute("data-active", "true");
}

function updateNameUsnMaskBox() {
    for (var i = 0; i < document.querySelectorAll(".backgroundNameUsnMask").length; i++) {
        settings.usnBackgroundMask[`${activeRole}`].value[i].name = document.querySelectorAll(".backgroundNameUsnMask")[i].value;
    }
}

function updateColorUsnMaskBox(type) {
    if (event.target.classList.contains("colorChangeOpacity")) {
        event.target.setAttribute("data-value", event.target.value);
    }
    
    settings.usnBackgroundMask[`${activeRole}`].value[activeUsnMaskGrad].colors[activeUsnMaskColor].opacity = parseInt(document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].querySelector(".colorChangeOpacity").value);
    updateColorSliderUsnMaskBox();

    decompileUsnMask();
}

function deleteColorUsnMaskBox() {
    settings.usnBackgroundMask[`${activeRole}`].value[activeUsnMaskGrad].colors.splice(activeUsnMaskColor, 1);
    document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].querySelectorAll(".colorSliderUsnMask")[activeUsnMaskColor].remove();
    updateColorSliderUsnMaskBox();
    for (let i = 0; i < document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].querySelectorAll(".colorSliderUsnMask").length; i++) {
        document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].querySelectorAll(".colorSliderUsnMask")[i].setAttribute("onpointerdown", `openColorUsnMaskBox(${activeUsnMaskGrad}, ${i})`);
    }
    document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[activeUsnMaskGrad].setAttribute("data-color-show", "false");
    activeUsnMaskGrad = undefined;
    activeUsnMaskColor = undefined;

    for (let j = 0; j < settings.usnBackgroundMask[`${activeRole}`].value.length; j++) {
        for (let k = 0; k < settings.usnBackgroundMask[`${activeRole}`].value[j].colors.length; k++) {
            document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[j].querySelectorAll(".colorSlider")[k].setAttribute("onpointerdown", `openColorUsnMaskBox(${j}, ${k})`)
        }
    }
...
function addColorSliderUsnMask(e) {
    i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode);
    document.querySelectorAll(".colorSlider").forEach(slider => {
        slider.setAttribute("data-active", "false");
    })
    var newSlider = document.createElement("input");newSlider.className = "colorSlider colorSliderUsnMask";newSlider.setAttribute("oninput", "updateColorSliderUsnMaskBox()");newSlider.setAttribute("onpointerdown", `openColorUsnMaskBox(${i}, ${e.target.parentNode.querySelectorAll(".colorSlider").length})`);newSlider.type = "range";newSlider.value = (e.layerX / e.target.clientWidth) * 100;newSlider.min = "0";newSlider.max = "100";newSlider.step = "0.1";newSlider.setAttribute("data-active", "true");
                
    document.querySelectorAll(".colorSliderUsnMaskCont")[i].appendChild(newSlider);

    settings.usnBackgroundMask[`${activeRole}`].value[i].colors.push({
        position: newSlider.value, // In %
        opacity: 255, // In Hex
        count: 0
    });
    updateColorSliderUsnMaskBox();
    openColorUsnMaskBox(i, e.target.parentNode.querySelectorAll(".colorSlider").length - 1);
...
function hideColorUsnMaskBox() {
    document.querySelectorAll(".colorSliderUsnMaskCont")[activeUsnMaskGrad].querySelectorAll(".colorSliderUsnMask")[activeUsnMaskColor].setAttribute("data-active", "false");
    activeUsnMaskGrad = undefined;
    activeUsnMaskColor = undefined;
    event.target.parentNode.parentNode.setAttribute("data-color-show", "false");
}

function switchSetUsnMaskBox() {
    var open = true;
...
function addGradientUsnMask(type, index) {
    var indexPlace;
    if (index === undefined) {
        indexPlace = 0;
        if (type === "solid") {
            settings.usnBackgroundMask[`${activeRole}`].value.splice(indexPlace, 0, {
                type: "solid",
                name: `Mask ${settings.usnBackgroundMask[`${activeRole}`].value.length + 1}`,
                colors: [
                    {
                        position:0,
                        opacity:255,
            ...
            var colorMenu = document.createElement("div");
                ...
                colorMenu.innerHTML = `
                    <div class="colorSliderAddRow">
                        <button class="colorSliderMoveUp icon" onclick="moveColorSliderUsnMask(event, 'up')" title="Moves this background one layer to the top"><img class="iconSVG" src="Icons/Diverse/UpLite.svg"></button>
                        <button class="colorSliderMoveDown icon" onclick="moveColorSliderUsnMask(event, 'down')" title="Moves this background one layer to the bottom"><img class="iconSVG" src="Icons/Diverse/DownLite.svg"></button>
                        <button class="colorSliderDuplicate icon" onclick="duplicateColorSliderUsnMask()" title="Duplicates this background to a new layer"><img class="iconSVG" src="Icons/Diverse/Copy.svg"></button>
                        <input class="backgroundNameUsnMask backgroundName" type="text" value="${settings.usnBackgroundMask[`${activeRole}`].value[0].name}" oninput="updateNameUsnMaskBox()">
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderUsnMaskCont" style="background-color:#ffffff;">
                                <input class="colorSlider colorSliderUsnMask" oninput="updateColorSliderUsnMaskBox()" onpointerdown="openColorUsnMaskBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                        ...
                        <input type="range" class="colorChangeOpacity" value="0" min="0" max="255" step="1" oninput="updateColorUsnMaskBox()" data-value="255" title="Changes the opacity of the selected color">
                        <button class="icon colorDeleteBtn" onclick="deleteColorUsnMaskBox()" title="Removes the selected color"><img class="iconSVG descrIconSVG" src="Icons/Diverse/Remove.svg"></button>
                    </div>
                    ...
                    <div class="bgSizeSettings">
                        <div class="numCont">
                            <span class="numContDescr">x-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateUsnMaskSizePos('posX', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateUsnMaskSizePos('posX', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">y-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateUsnMaskSizePos('posY', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateUsnMaskSizePos('posY', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">width:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateUsnMaskSizePos('width', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateUsnMaskSizePos('width', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">height:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateUsnMaskSizePos('height', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateUsnMaskSizePos('height', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                    </div>
                `;
                ...
                colorMenu.innerHTML = `
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderUsnMaskCont" style="background-image: linear-gradient(90deg, #000000 0%, #ffffff 100%);">
                                <input class="colorSlider colorSliderUsnMask" oninput="updateColorSliderUsnMaskBox()" onpointerdown="openColorUsnMaskBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                                <input class="colorSlider colorSliderUsnMask" oninput="updateColorSliderUsnMaskBox()" onpointerdown="openColorUsnMaskBox(0, 1)" type="range" value="100" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                    ...
                    <button class="showGradSpecBtn" onclick="switchSetUsnMaskBox()" data-active="false" title="Opens the settings specific to this type of gradient"><img class="iconSVG" src="Icons/Diverse/Settings.svg"></button>
                    <div class="gradSpecSettings">
                        <div class="inputNumRow">
                            <div class="numCont">
                                <span class="numContDescr">rotation:</span>
                                <input class="numContInput" type="number" value="90" oninput="updateLinearRotUsnMask(event)">
                                <span class="numContUnit">°</span>
                            </div>
                        </div>
                    </div>
                ...
                colorMenu.innerHTML = `
                    ...
                    <div class="gradSpecSettings">
                        <div class="singleChoiceList gradRadialPosContUsnMask">
                            <button data-active="true" onclick="updateRadialTypeUsnMask(event, 'farthest-corner')">Farthest Corner</button>
                            <button data-active="false" onclick="updateRadialTypeUsnMask(event, 'farthest-side')">Farthest Edge</button>
                            <button data-active="false" onclick="updateRadialTypeUsnMask(event, 'closest-corner')">Closest Corner</button>
                            <button data-active="false" onclick="updateRadialTypeUsnMask(event, 'closest-side')">Closest Edge</button>
                            <button data-active="false" onclick="updateRadialTypeUsnMask(event, 'circle')">Circle</button>
                        </div>
                        <div class="inputNumRow">
                            <div class="numCont">
                                <span class="numContDescr">x:</span>
                                <input class="numContInput" type="number" value="50" oninput="updateRadialPosUsnMask(event, 'x')">
                                <span class="numContUnit">%</span>
                            </div>
                            <div class="numCont">
                                <span class="numContDescr">y:</span>
                                <input class="numContInput" type="number" value="50" oninput="updateRadialPosUsnMask(event, 'y')">
                                <span class="numContUnit">%</span>
                            </div>
                        </div>
                    </div>
        ...
        document.querySelector(".colorMenuColUsnMask").insertBefore(colorMenu, document.querySelector(".colorMenuColUsnMask").children[indexPlace]);
    } else {
        indexPlace = index;

        var menuOpt = JSON.parse(JSON.stringify(settings.usnBackgroundMask[`${activeRole}`].value[indexPlace]));
        settings.usnBackgroundMask[`${activeRole}`].value.splice(indexPlace, 0, menuOpt);

        var colorMenuNode = document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[indexPlace].cloneNode(true);
        colorMenuNode.setAttribute("data-color-show", "false");
        colorMenuNode.setAttribute("data-set-show", "false");
        colorMenuNode.setAttribute("data-size-show", "false");
        document.querySelector(".colorMenuColUsnMask").insertBefore(colorMenuNode, document.querySelector(".colorMenuColUsnMask").children[indexPlace]);
    }
    
    for (var i = 0; i < document.querySelectorAll(".colorSliderUsnMaskCont").length; i++) {
        for (var j = 0; j < document.querySelectorAll(".colorSliderUsnMaskCont")[i].querySelectorAll(".colorSlider").length; j++) {
            document.querySelectorAll(".colorSliderUsnMaskCont")[i].querySelectorAll(".colorSlider")[j].setAttribute("onpointerdown", `openColorUsnMaskBox(${i}, ${j})`);
        }
    }
...
function removeGradientUsnMask(e) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode);
    if (i < activeUsnMaskGrad) {
        activeUsnMaskGrad -= 1;
    }
    settings.usnBackgroundMask[`${activeRole}`].value.splice(i, 1);
    document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[i].remove();
    for (let j = 0; j < document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu").length; j++) {
        for (let k = 0; k < document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[j].querySelectorAll(".colorSlider").length; k++) {
            document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[j].querySelectorAll(".colorSlider")[k].setAttribute("onpointerdown", `openColorUsnMaskBox(${j}, ${k})`)
        }
    }
...
function updateRadialTypeUsnMask(e, size) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode);
    settings.usnBackgroundMask[`${activeRole}`].value[i].size = size;
    for (let j = 0; j < settings.usnBackgroundMask[`${activeRole}`].value.length; j++) {
        if (settings.usnBackgroundMask[`${activeRole}`].value[j].type === "radial") {
            for (let k = 0; k < document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[j].querySelector(".gradRadialPosContUsnMask").querySelectorAll("button").length; k++) {
                document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[j].querySelector(".gradRadialPosContUsnMask").querySelectorAll("button")[k].setAttribute("data-active", "false");
            }
            e.target.setAttribute("data-active", "true");
        }
    }
...
function updateLinearRotUsnMask(e) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode.parentNode);
    settings.usnBackgroundMask[`${activeRole}`].value[i].rotate = parseInt(e.target.value);
    
    decompileUsnMask();
}

function updateRadialPosUsnMask(e, xy) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode.parentNode);
    if (xy === "x") {
        settings.usnBackgroundMask[`${activeRole}`].value[i].x = parseInt(e.target.value);
    } else if (xy === "y") {
        settings.usnBackgroundMask[`${activeRole}`].value[i].y = parseInt(e.target.value);
    }
    
    decompileUsnMask();
}

function moveColorSliderUsnMask(e, dir) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode);
    if (dir === "up" && i > 0) {
        var elmnt = settings.usnBackgroundMask[`${activeRole}`].value[i];
        settings.usnBackgroundMask[`${activeRole}`].value.splice(i, 1);
        settings.usnBackgroundMask[`${activeRole}`].value.splice(i - 1, 0, elmnt);
        document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[i - 1].before(document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[i]);
        if (activeUsnMaskGrad === i) {
            activeUsnMaskGrad -= 1;
        } else if (activeUsnMaskGrad === i - 1) {
            activeUsnMaskGrad += 1;
        }
    } else if (dir === "down" && i < settings.usnBackgroundMask[`${activeRole}`].value.length - 1) {
        var elmnt = settings.usnBackgroundMask[`${activeRole}`].value[i];
        settings.usnBackgroundMask[`${activeRole}`].value.splice(i, 1);
        settings.usnBackgroundMask[`${activeRole}`].value.splice(i + 1, 0, elmnt);
        document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[i + 1].after(document.querySelector(".colorMenuColUsnMask").querySelectorAll(".colorMenu")[i]);
        if (activeUsnMaskGrad === i) {
            activeUsnMaskGrad += 1;
        } else if (activeUsnMaskGrad === i + 1) {
            activeUsnMaskGrad -= 1;
...
function duplicateColorSliderUsnMask() {
    activeUsnMaskGrad += 1;
    var i = Array.from(event.target.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode);
    var type = settings.usnBackgroundMask[`${activeRole}`].value[i].type;

    addGradientUsnMask(type, i);
}

function updateUsnMaskSizePos(cat, value) {
    var i = Array.from(event.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode.parentNode);
    if (cat === "posX") {
        if (value === "%") {
            settings.usnBackgroundMask[`${activeRole}`].value[i].posX.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.usnBackgroundMask[`${activeRole}`].value[i].posX.px = parseInt(event.target.value);
        }
    } else if (cat === "posY") {
        if (value === "%") {
            settings.usnBackgroundMask[`${activeRole}`].value[i].posY.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.usnBackgroundMask[`${activeRole}`].value[i].posY.px = parseInt(event.target.value);
        }
    } else if (cat === "width") {
        if (value === "%") {
            settings.usnBackgroundMask[`${activeRole}`].value[i].width.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.usnBackgroundMask[`${activeRole}`].value[i].width.px = parseInt(event.target.value);
        }
    } else if (cat === "height") {
        if (value === "%") {
            settings.usnBackgroundMask[`${activeRole}`].value[i].height.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.usnBackgroundMask[`${activeRole}`].value[i].height.px = parseInt(event.target.value);
        }
    }
        ...
        if (settings.usnBorder[`${role}`].active === true) {
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                if (settings.usnBorder[`${role}`].value.top.color.type === "custom") {
                    ...
                    usn.style.setProperty(`--usnBorderTop`, `${settings.usnBorder[`${role}`].value.top.type} ${settings.usnBorder[`${role}`].value.top.width}px ${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.usnBorder[`${role}`].value.top.color.opacity)}`);
                }
                ...
                if (settings.usnBorder[`${role}`].value.right.color.type === "custom") {
                    ...
                    usn.style.setProperty(`--usnBorderRight`, `${settings.usnBorder[`${role}`].value.right.type} ${settings.usnBorder[`${role}`].value.right.width}px ${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.usnBorder[`${role}`].value.right.color.opacity)}`);
                }
                ...
                if (settings.usnBorder[`${role}`].value.bottom.color.type === "custom") {
                    ...
                    usn.style.setProperty(`--usnBorderBottom`, `${settings.usnBorder[`${role}`].value.bottom.type} ${settings.usnBorder[`${role}`].value.bottom.width}px ${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.usnBorder[`${role}`].value.bottom.color.opacity)}`);
                }
                ...
                if (settings.usnBorder[`${role}`].value.left.color.type === "custom") {
                    ...
                    usn.style.setProperty(`--usnBorderLeft`, `${settings.usnBorder[`${role}`].value.left.type} ${settings.usnBorder[`${role}`].value.left.width}px ${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.usnBorder[`${role}`].value.left.color.opacity)}`);
                }
            });
        }
    });
            ...
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                for (let i = 0; i < settings.usnShadow[`${role}`].value.length; i++) {
                    ...
                    if (settings.usnShadow[`${role}`].value[i].color.type === "custom") {
                        ...
                        usnShadow += `${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.usnShadow[`${role}`].value[i].color.opacity)}`;
                    }
...
function openShadowUsn(triggerAdd) {
    ...
    if (activeUsnShadow >= 0) {
        ...
        document.querySelector(".shadowSettingsUsn").querySelectorAll(".numContInput")[0].value = settings.usnShadow[`${activeRole}`].value[activeUsnShadow].x;
        document.querySelector(".shadowSettingsUsn").querySelectorAll(".numContInput")[1].value = settings.usnShadow[`${activeRole}`].value[activeUsnShadow].y;
        ...
        document.querySelector("#usnShadowOpacity").setAttribute("data-value", settings.usnShadow[`${activeRole}`].value[activeUsnShadow].color.opacity);
...
function updateShadowUsn(type, value) {
    ...
    settings.usnShadow[`${activeRole}`].value[activeUsnShadow].x    = parseInt(document.querySelector(".shadowSettingsUsn").querySelectorAll(".numContInput")[0].value);
    settings.usnShadow[`${activeRole}`].value[activeUsnShadow].y    = parseInt(document.querySelector(".shadowSettingsUsn").querySelectorAll(".numContInput")[1].value);
    ...
    document.querySelector("#usnShadowOpacity").setAttribute("data-value", document.querySelector("#usnShadowOpacity").value);
...
function updateUsnSizingH(type) {
    if (type === "stretch") {
        event.target.parentNode.querySelectorAll("button")[0].setAttribute("data-active", "true");
        event.target.parentNode.querySelectorAll("button")[1].setAttribute("data-active", "false");
        event.target.parentNode.querySelectorAll("button")[2].setAttribute("data-active", "false");
    ...
    settings.usnPosition[`${activeRole}`].value.sizingH = type;

    decompileUsnSizing();
}

function updateUsnSizingV(type) {
    if (type === "stretch") {
        event.target.parentNode.querySelectorAll("button")[0].setAttribute("data-active", "true");
        event.target.parentNode.querySelectorAll("button")[1].setAttribute("data-active", "false");
        event.target.parentNode.querySelectorAll("button")[2].setAttribute("data-active", "false");
    ...
    settings.usnPosition[`${activeRole}`].value.sizingV = type;
        ...
        if (settings.usnFont[`${role}`].active === true) {
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                if (settings.usnFont[`${role}`].value.color.type === "custom") {
                    ...
                    usn.style.setProperty(`--usnFontColor`, `${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.usnFont[`${role}`].value.color.opacity)}`);
                }
...
function updateUsnFont(type, value) {
    ...
    if (type === "style") {
        ...
        settings.usnFont[`${activeRole}`].value.hAlign = event.target.getAttribute("data-value");
            ...
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                for (let i = 0; i < settings.usnFontShadow[`${role}`].value.length; i++) {
                    ...
                    if (settings.usnFontShadow[`${role}`].value[i].color.type === "custom") {
                        ...
                        usnFontShadow += `${usn.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.usnFontShadow[`${role}`].value[i].color.opacity)}`;
                    }
...
function openFontShadowUsn(triggerAdd) {
    ...
    if (activeUsnFontShadow >= 0) {
        ...
        document.querySelector(".fontShadowSettingsUsn").querySelectorAll(".numContInput")[0].value = settings.usnFontShadow[`${activeRole}`].value[activeUsnFontShadow].x;
        document.querySelector(".fontShadowSettingsUsn").querySelectorAll(".numContInput")[1].value = settings.usnFontShadow[`${activeRole}`].value[activeUsnFontShadow].y;
        ...
        document.querySelector("#usnFontShadowOpacity").setAttribute("data-value", settings.usnFontShadow[`${activeRole}`].value[activeUsnFontShadow].color.opacity);
...
function updateFontShadowUsn(type, value) {
    ...
    settings.usnFontShadow[`${activeRole}`].value[activeUsnFontShadow].x = parseInt(document.querySelector(".fontShadowSettingsUsn").querySelectorAll(".numContInput")[0].value);
    settings.usnFontShadow[`${activeRole}`].value[activeUsnFontShadow].y = parseInt(document.querySelector(".fontShadowSettingsUsn").querySelectorAll(".numContInput")[1].value);
    ...
    for (let i = 0; i < settings.usnFontShadow[`${activeRole}`].value.length; i++) {
        var usnStyle = `${settings.usnFontShadow[`${activeRole}`].value[i].x}px ${settings.usnFontShadow[`${activeRole}`].value[i].y}px ${settings.usnFontShadow[`${activeRole}`].value[i].blur}px ${settings.usnFontShadow[`${activeRole}`].value[i].color.color}${dec2hex(settings.usnFontShadow[`${activeRole}`].value[i].color.opacity)}`;
    ...
    roleList.forEach(role => {
        if (settings.usnContent[`${role}`].active === true) {
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                usnContentFinal = usnContentFinal.replace(/{usn}/g, usn.parentNode.parentNode.getAttribute("data-username"));

                usnContentFinal = usnContentFinal.replace(/{msg}/g, usn.parentNode.parentNode.getAttribute("data-message"));

                usnContentFinal = usnContentFinal.replace(/{streamer}/g, usn.parentNode.parentNode.getAttribute("data-streamer"));
                
                usnContentFinal = usnContentFinal.replace(/{h}/g, usn.parentNode.parentNode.getAttribute("data-h"));
                
                usnContentFinal = usnContentFinal.replace(/{hh}/g, usn.parentNode.parentNode.getAttribute("data-hh"));

                usnContentFinal = usnContentFinal.replace(/{h12}/g, usn.parentNode.parentNode.getAttribute("data-h12"));

                usnContentFinal = usnContentFinal.replace(/{hh12}/g, usn.parentNode.parentNode.getAttribute("data-hh12"));

                usnContentFinal = usnContentFinal.replace(/{apm}/g, usn.parentNode.parentNode.getAttribute("data-apm"));
                
                usnContentFinal = usnContentFinal.replace(/{m}/g, usn.parentNode.parentNode.getAttribute("data-m"));
                
                usnContentFinal = usnContentFinal.replace(/{mm}/g, usn.parentNode.parentNode.getAttribute("data-mm"));
                
                usnContentFinal = usnContentFinal.replace(/{s}/g, usn.parentNode.parentNode.getAttribute("data-s"));
                
                usnContentFinal = usnContentFinal.replace(/{ss}/g, usn.parentNode.parentNode.getAttribute("data-ss"));
                ...
                for (let i = 0; i < settings.msgBackground[`${role}`].value.length; i++) {
                    if (settings.msgBackground[`${role}`].value[i].type === "solid") {
                        if (settings.msgBackground[`${role}`].value[i].colors[0].type === "custom") {
                            ...
                            msgBackground += `linear-gradient(${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.msgBackground[`${role}`].value[i].colors[0].opacity)}, ${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.msgBackground[`${role}`].value[i].colors[0].opacity)})`;
                        }
                        ...
                        for (let j = 0; j < sortedColors.length; j++) {
                            if (sortedColors[j].type === "custom") {
                                ...
                                msgBackground += `${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(sortedColors[j].opacity)} ${sortedColors[j].position}%`
                            }
...
function openColorMsgBGBox(grdNum, clrNum) {
    ...
    document.querySelector(".colorMenuColMsgBG").querySelectorAll(".colorMenu")[activeMsgBGGrad].setAttribute("data-set-show", "false");
    document.querySelector(".colorMenuColMsgBG").querySelectorAll(".colorMenu")[activeMsgBGGrad].setAttribute("data-size-show", "false");
    ...
    document.querySelector(".colorMenuColMsgBG").querySelectorAll(".colorMenu")[activeMsgBGGrad].querySelector(".colorChangeOpacity").setAttribute("data-value", settings.msgBackground[`${activeRole}`].value[activeMsgBGGrad].colors[activeMsgBGColor].opacity);
    document.querySelectorAll(".colorSliderMsgBG").forEach(msgCont => {
        msgCont.setAttribute("data-active", "false");
    });
    document.querySelectorAll(".colorSliderMsgBGCont")[activeMsgBGGrad].querySelectorAll(".colorSliderMsgBG")[activeMsgBGColor].setAttribute("data-active", "true");
}

function updateNameMsgBGBox() {
    for (var i = 0; i < document.querySelectorAll(".backgroundNameMsgBG").length; i++) {
        settings.msgBackground[`${activeRole}`].value[i].name = document.querySelectorAll(".backgroundNameMsgBG")[i].value;
    }
}
...
function hideColorMsgBGBox() {
    document.querySelectorAll(".colorSliderMsgBGCont")[activeMsgBGGrad].querySelectorAll(".colorSliderMsgBG")[activeMsgBGColor].setAttribute("data-active", "false");
    activeMsgBGGrad = undefined;
    activeMsgBGColor = undefined;
    event.target.parentNode.parentNode.setAttribute("data-color-show", "false");
}

function switchSetMsgBGBox() {
    var open = true;
...
function addGradientMsgBG(type, index) {
    var indexPlace;
    if (index === undefined) {
        indexPlace = 0;
        if (type === "solid") {
            settings.msgBackground[`${activeRole}`].value.splice(indexPlace, 0, {
                type: "solid",
                name: `Background ${settings.msgBackground[`${activeRole}`].value.length + 1}`,
                colors: [
                    {
                        position:0,
                        type: "custom",
            ...
            var colorMenu = document.createElement("div");
                ...
                colorMenu.innerHTML = `
                    <div class="colorSliderAddRow">
                        <button class="colorSliderMoveUp icon" onclick="moveColorSliderMsgBG(event, 'up')" title="Moves this background one layer to the top"><img class="iconSVG" src="Icons/Diverse/UpLite.svg"></button>
                        <button class="colorSliderMoveDown icon" onclick="moveColorSliderMsgBG(event, 'down')" title="Moves this background one layer to the bottom"><img class="iconSVG" src="Icons/Diverse/DownLite.svg"></button>
                        <button class="colorSliderDuplicate icon" onclick="duplicateColorSliderMsgBG()" title="Duplicates this background to a new layer"><img class="iconSVG" src="Icons/Diverse/Copy.svg"></button>
                        <input class="backgroundNameMsgBG backgroundName" type="text" value="${settings.msgBackground[`${activeRole}`].value[0].name}" oninput="updateNameMsgBGBox()">
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderMsgBGCont" style="background-color:#ffffff;">
                                <input class="colorSlider colorSliderMsgBG" oninput="updateColorSliderMsgBGBox()" onpointerdown="openColorMsgBGBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                        <button class="hideColorBtn" onclick="hideColorMsgBGBox()" title="Hides the color panel"><img class="iconSVG" src="Icons/Diverse/UpLite.svg"></button>
                        <div class="colorChangeType selectItemRow">
                            ...
                            <button class="icon" onclick="updateColorMsgBGBox('twitch')" title="Uses the color the user uses in the Twitch chat"><img class="iconSVG descrIconSVG" src="Icons/Diverse/TwitchColor.svg"></button>
                            <button class="icon colorDeleteBtn" onclick="deleteColorMsgBGBox()" title="Removes the selected color"><img class="iconSVG descrIconSVG" src="Icons/Diverse/Remove.svg"></button>
                        </div>
                        <input type="range" class="colorChangeOpacity" value="0" min="0" max="255" step="1" oninput="updateColorMsgBGBox()" data-value="255" title="Changes the opacity of the selected color">
                        <div class="presetCont">
                            <div class="presetList" data-delete-show="false">${insertPresetColor()}</div>
                            <button class="presetAdd" onclick="addPreset()"><img class="iconSVG" src="Icons/Diverse/Add.svg"></button>
                            <button class="presetShowDelete" onclick="showDeletePreset()"><img class="iconSVG" src="Icons/Diverse/Remove.svg"></button>
                    ...
                    <div class="bgSizeSettings">
                        <div class="numCont">
                            <span class="numContDescr">x-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateMsgBGSizePos('posX', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateMsgBGSizePos('posX', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">y-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateMsgBGSizePos('posY', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateMsgBGSizePos('posY', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">width:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateMsgBGSizePos('width', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateMsgBGSizePos('width', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">height:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateMsgBGSizePos('height', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateMsgBGSizePos('height', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                    </div>
                `;
            ...
            var colorMenu = document.createElement("div");
                ...
                colorMenu.innerHTML = `
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderMsgBGCont" style="background-image: linear-gradient(90deg, #000000 0%, #ffffff 100%);">
                                <input class="colorSlider colorSliderMsgBG" oninput="updateColorSliderMsgBGBox()" onpointerdown="openColorMsgBGBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                                <input class="colorSlider colorSliderMsgBG" oninput="updateColorSliderMsgBGBox()" onpointerdown="openColorMsgBGBox(0, 1)" type="range" value="100" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                    ...
                    <button class="showGradSpecBtn" onclick="switchSetMsgBGBox()" data-active="false" title="Opens the settings specific to this type of gradient"><img class="iconSVG" src="Icons/Diverse/Settings.svg"></button>
                    <div class="gradSpecSettings">
                        <div class="inputNumRow">
                            <div class="numCont">
                                <span class="numContDescr">rotation:</span>
        ...
        document.querySelector(".colorMenuColMsgBG").prepend(colorMenu);
    } else {
        indexPlace = index;

        var menuOpt = JSON.parse(JSON.stringify(settings.msgBackground[`${activeRole}`].value[indexPlace]));
        settings.msgBackground[`${activeRole}`].value.splice(indexPlace, 0, menuOpt);

        var colorMenuNode = document.querySelector(".colorMenuColMsgBG").querySelectorAll(".colorMenu")[indexPlace].cloneNode(true);
        colorMenuNode.setAttribute("data-color-show", "false");
        colorMenuNode.setAttribute("data-set-show", "false");
        colorMenuNode.setAttribute("data-size-show", "false");
        document.querySelector(".colorMenuColMsgBG").insertBefore(colorMenuNode, document.querySelector(".colorMenuColMsgBG").children[indexPlace]);
    }
    
    for (var i = 0; i < document.querySelectorAll(".colorSliderMsgBGCont").length; i++) {
        for (var j = 0; j < document.querySelectorAll(".colorSliderMsgBGCont")[i].querySelectorAll(".colorSlider").length; j++) {
            document.querySelectorAll(".colorSliderMsgBGCont")[i].querySelectorAll(".colorSlider")[j].setAttribute("onpointerdown", `openColorMsgBGBox(${i}, ${j})`);
        }
    }
...
function updateRadialPosMsgBG(e, xy) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode.parentNode);
    if (xy === "x") {
        settings.msgBackground[`${activeRole}`].value[i].x = parseInt(e.target.value);
    } else if (xy === "y") {
        settings.msgBackground[`${activeRole}`].value[i].y = parseInt(e.target.value);
    }
...
function duplicateColorSliderMsgBG() {
    activeMsgBGGrad += 1;
    var i = Array.from(event.target.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode);
    var type = settings.msgBackground[`${activeRole}`].value[i].type;

    addGradientMsgBG(type, i);
}

function updateMsgBGSizePos(cat, value) {
    var i = Array.from(event.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode.parentNode);
    if (cat === "posX") {
        if (value === "%") {
            settings.msgBackground[`${activeRole}`].value[i].posX.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.msgBackground[`${activeRole}`].value[i].posX.px = parseInt(event.target.value);
        }
    } else if (cat === "posY") {
        if (value === "%") {
            settings.msgBackground[`${activeRole}`].value[i].posY.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.msgBackground[`${activeRole}`].value[i].posY.px = parseInt(event.target.value);
        }
    } else if (cat === "width") {
        if (value === "%") {
            settings.msgBackground[`${activeRole}`].value[i].width.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.msgBackground[`${activeRole}`].value[i].width.px = parseInt(event.target.value);
        }
    } else if (cat === "height") {
        if (value === "%") {
            settings.msgBackground[`${activeRole}`].value[i].height.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.msgBackground[`${activeRole}`].value[i].height.px = parseInt(event.target.value);
        }
    }
...
// settings.msgBackgroundMask

var activeMsgMaskGrad = 0;
var activeMsgMaskColor = 0;

function decompileMsgMask() {
    roleList.forEach(role => {
...
function updateColorSliderMsgMaskBox() {
    for (let i = 0; i < document.querySelectorAll(".colorSliderMsgMaskCont").length; i++) {
        if (settings.msgBackgroundMask[`${activeRole}`].value[i].type !== "solid") {
            
            var msgMaskPrev = "linear-gradient(90deg, ",
                nodeList = document.querySelectorAll(".colorSliderMsgMaskCont")[i].querySelectorAll(".colorSliderMsgMask"),
                colorList = settings.msgBackgroundMask[`${activeRole}`].value[i].colors;

            for (let j = 0; j < settings.msgBackgroundMask[`${activeRole}`].value[i].colors.length; j++) {
                settings.msgBackgroundMask[`${activeRole}`].value[i].colors[j].count = 0;
                colorList[j].position = parseFloat(nodeList[j].value);
                nodeList[j].style.setProperty("--value", `${nodeList[j].value}%`);
                nodeList[j].style.setProperty("--valueTxt", `"${nodeList[j].value}"`);
            }
            ...
            for (let j = 0; j < settings.msgBackgroundMask[`${activeRole}`].value[i].colors.length; j++) {
                for (let k = 0; k < settings.msgBackgroundMask[`${activeRole}`].value[i].colors.length; k++) {
                    if (k !== j) {
                        if (settings.msgBackgroundMask[`${activeRole}`].value[i].colors[j].position >= settings.msgBackgroundMask[`${activeRole}`].value[i].colors[k].position) {
                            settings.msgBackgroundMask[`${activeRole}`].value[i].colors[j].count += 1;
                        }
                    }
                }
            }
            ...
            for (let j = 0; j < settings.msgBackgroundMask[`${activeRole}`].value[i].colors.length; j++) {
                var resultElmnt = settings.msgBackgroundMask[`${activeRole}`].value[i].colors.find(findElmnt => findElmnt.count === j);
                msgMaskPrev += `#ffffff${dec2hex(resultElmnt.opacity)} ${resultElmnt.position}%`;
                
                if (j < settings.msgBackgroundMask[`${activeRole}`].value[i].colors.length - 1) {
                    msgMaskPrev += ", ";
                } else {
                    msgMaskPrev += ")";
                }
            ...
            document.querySelectorAll(".colorSliderMsgMaskCont")[i].style.backgroundImage = msgMaskPrev;
        } else {
            var msgMaskPrev = `#ffffff${dec2hex(settings.msgBackgroundMask[`${activeRole}`].value[i].colors[0].opacity)}`;
            document.querySelectorAll(".colorSliderMsgMaskCont")[i].style.backgroundColor = msgMaskPrev;
        }
    }
    decompileMsgMask();
}

function openColorMsgMaskBox(grdNum, clrNum) {
    var open = true;
    ...
    activeMsgMaskGrad = grdNum;
    activeMsgMaskColor = clrNum;
    document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].setAttribute("data-color-show", "true");
    document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].setAttribute("data-set-show", "false");
    document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].setAttribute("data-size-show", "false");
    document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].querySelector(".colorChangeOpacity").value = settings.msgBackgroundMask[`${activeRole}`].value[activeMsgMaskGrad].colors[activeMsgMaskColor].opacity;
    document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].querySelector(".colorChangeOpacity").setAttribute("data-value", settings.msgBackgroundMask[`${activeRole}`].value[activeMsgMaskGrad].colors[activeMsgMaskColor].opacity);
    document.querySelectorAll(".colorSliderMsgMask").forEach(msgCont => {
        msgCont.setAttribute("data-active", "false");
    });
    document.querySelectorAll(".colorSliderMsgMaskCont")[activeMsgMaskGrad].querySelectorAll(".colorSliderMsgMask")[activeMsgMaskColor].setAttribute("data-active", "true");
}

function updateNameMsgMaskBox() {
    for (var i = 0; i < document.querySelectorAll(".backgroundNameMsgMask").length; i++) {
        settings.msgBackgroundMask[`${activeRole}`].value[i].name = document.querySelectorAll(".backgroundNameMsgMask")[i].value;
    }
}

function updateColorMsgMaskBox(type) {
    if (event.target.classList.contains("colorChangeOpacity")) {
        event.target.setAttribute("data-value", event.target.value);
    }
    
    settings.msgBackgroundMask[`${activeRole}`].value[activeMsgMaskGrad].colors[activeMsgMaskColor].opacity = parseInt(document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].querySelector(".colorChangeOpacity").value);
    updateColorSliderMsgMaskBox();

    decompileMsgMask();
}

function deleteColorMsgMaskBox() {
    settings.msgBackgroundMask[`${activeRole}`].value[activeMsgMaskGrad].colors.splice(activeMsgMaskColor, 1);
    document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].querySelectorAll(".colorSliderMsgMask")[activeMsgMaskColor].remove();
    updateColorSliderMsgMaskBox();
    for (let i = 0; i < document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].querySelectorAll(".colorSliderMsgMask").length; i++) {
        document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].querySelectorAll(".colorSliderMsgMask")[i].setAttribute("onpointerdown", `openColorMsgMaskBox(${activeMsgMaskGrad}, ${i})`);
    }
    document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[activeMsgMaskGrad].setAttribute("data-color-show", "false");
    activeMsgMaskGrad = undefined;
    activeMsgMaskColor = undefined;

    for (let j = 0; j < settings.msgBackgroundMask[`${activeRole}`].value.length; j++) {
        for (let k = 0; k < settings.msgBackgroundMask[`${activeRole}`].value[j].colors.length; k++) {
            document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[j].querySelectorAll(".colorSlider")[k].setAttribute("onpointerdown", `openColorMsgMaskBox(${j}, ${k})`)
        }
    }
...
function addColorSliderMsgMask(e) {
    i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode);
    document.querySelectorAll(".colorSlider").forEach(slider => {
        slider.setAttribute("data-active", "false");
    })
    var newSlider = document.createElement("input");newSlider.className = "colorSlider colorSliderMsgMask";newSlider.setAttribute("oninput", "updateColorSliderMsgMaskBox()");newSlider.setAttribute("onpointerdown", `openColorMsgMaskBox(${i}, ${e.target.parentNode.querySelectorAll(".colorSlider").length})`);newSlider.type = "range";newSlider.value = (e.layerX / e.target.clientWidth) * 100;newSlider.min = "0";newSlider.max = "100";newSlider.step = "0.1";newSlider.setAttribute("data-active", "true");
                
    document.querySelectorAll(".colorSliderMsgMaskCont")[i].appendChild(newSlider);

    settings.msgBackgroundMask[`${activeRole}`].value[i].colors.push({
        position: newSlider.value, // In %
        opacity: 255, // In Hex
        count: 0
    });
    updateColorSliderMsgMaskBox();
    openColorMsgMaskBox(i, e.target.parentNode.querySelectorAll(".colorSlider").length - 1);
...
function hideColorMsgMaskBox() {
    document.querySelectorAll(".colorSliderMsgMaskCont")[activeMsgMaskGrad].querySelectorAll(".colorSliderMsgMask")[activeMsgMaskColor].setAttribute("data-active", "false");
    activeMsgMaskGrad = undefined;
    activeMsgMaskColor = undefined;
    event.target.parentNode.parentNode.setAttribute("data-color-show", "false");
}

function switchSetMsgMaskBox() {
    var open = true;
...
function addGradientMsgMask(type, index) {
    var indexPlace;
    if (index === undefined) {
        indexPlace = 0;
        if (type === "solid") {
            settings.msgBackgroundMask[`${activeRole}`].value.splice(indexPlace, 0, {
                type: "solid",
                name: `Mask ${settings.msgBackgroundMask[`${activeRole}`].value.length + 1}`,
                colors: [
                    {
                        position:0,
                        opacity:255,
            ...
            var colorMenu = document.createElement("div");
                ...
                colorMenu.innerHTML = `
                    <div class="colorSliderAddRow">
                        <button class="colorSliderMoveUp icon" onclick="moveColorSliderMsgMask(event, 'up')" title="Moves this background one layer to the top"><img class="iconSVG" src="Icons/Diverse/UpLite.svg"></button>
                        <button class="colorSliderMoveDown icon" onclick="moveColorSliderMsgMask(event, 'down')" title="Moves this background one layer to the bottom"><img class="iconSVG" src="Icons/Diverse/DownLite.svg"></button>
                        <button class="colorSliderDuplicate icon" onclick="duplicateColorSliderMsgMask()" title="Duplicates this background to a new layer"><img class="iconSVG" src="Icons/Diverse/Copy.svg"></button>
                        <input class="backgroundNameMsgMask backgroundName" type="text" value="${settings.msgBackgroundMask[`${activeRole}`].value[0].name}" oninput="updateNameMsgMaskBox()">
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderMsgMaskCont" style="background-color:#ffffff;">
                                <input class="colorSlider colorSliderMsgMask" oninput="updateColorSliderMsgMaskBox()" onpointerdown="openColorMsgMaskBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                        ...
                        <input type="range" class="colorChangeOpacity" value="0" min="0" max="255" step="1" oninput="updateColorMsgMaskBox()" data-value="255" title="Changes the opacity of the selected color">
                        <button class="icon colorDeleteBtn" onclick="deleteColorMsgMaskBox()" title="Removes the selected color"><img class="iconSVG descrIconSVG" src="Icons/Diverse/Remove.svg"></button>
                    </div>
                    ...
                    <div class="bgSizeSettings">
                        <div class="numCont">
                            <span class="numContDescr">x-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateMsgMaskSizePos('posX', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateMsgMaskSizePos('posX', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">y-position:</span>
                            <input class="numContInput" type="number" value="50" oninput="updateMsgMaskSizePos('posY', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateMsgMaskSizePos('posY', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">width:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateMsgMaskSizePos('width', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateMsgMaskSizePos('width', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                        <div class="numCont">
                            <span class="numContDescr">height:</span>
                            <input class="numContInput" type="number" value="100" oninput="updateMsgMaskSizePos('height', '%')">
                            <span class="numContUnit">%</span>
                            +
                            <input class="numContInput" type="number" value="0" oninput="updateMsgMaskSizePos('height', 'px')">
                            <span class="numContUnit">px</span>
                        </div>
                    </div>
                `;
                ...
                colorMenu.innerHTML = `
                    ...
                    <div class="colorSliderAddRow">
                        <div class="colorSliderAddCont">
                            ...
                            <div class="colorSliderCont colorSliderMsgMaskCont" style="background-image: linear-gradient(90deg, #000000 0%, #ffffff 100%);">
                                <input class="colorSlider colorSliderMsgMask" oninput="updateColorSliderMsgMaskBox()" onpointerdown="openColorMsgMaskBox(0, 0)" type="range" value="0" min="0" max="100" step="0.1">
                                <input class="colorSlider colorSliderMsgMask" oninput="updateColorSliderMsgMaskBox()" onpointerdown="openColorMsgMaskBox(0, 1)" type="range" value="100" min="0" max="100" step="0.1">
                            </div>
                        </div>
                    </div>
                    <div class="colorChangeCont">
                    ...
                    <button class="showGradSpecBtn" onclick="switchSetMsgMaskBox()" data-active="false" title="Opens the settings specific to this type of gradient"><img class="iconSVG" src="Icons/Diverse/Settings.svg"></button>
                    <div class="gradSpecSettings">
                        <div class="inputNumRow">
                            <div class="numCont">
                                <span class="numContDescr">rotation:</span>
                                <input class="numContInput" type="number" value="90" oninput="updateLinearRotMsgMask(event)">
                                <span class="numContUnit">°</span>
                            </div>
                        </div>
                    </div>
                ...
                colorMenu.innerHTML = `
                    ...
                    <div class="gradSpecSettings">
                        <div class="singleChoiceList gradRadialPosContMsgMask">
                            <button data-active="true" onclick="updateRadialTypeMsgMask(event, 'farthest-corner')">Farthest Corner</button>
                            <button data-active="false" onclick="updateRadialTypeMsgMask(event, 'farthest-side')">Farthest Edge</button>
                            <button data-active="false" onclick="updateRadialTypeMsgMask(event, 'closest-corner')">Closest Corner</button>
                            <button data-active="false" onclick="updateRadialTypeMsgMask(event, 'closest-side')">Closest Edge</button>
                            <button data-active="false" onclick="updateRadialTypeMsgMask(event, 'circle')">Circle</button>
                        </div>
                        <div class="inputNumRow">
                            <div class="numCont">
                                <span class="numContDescr">x:</span>
                                <input class="numContInput" type="number" value="50" oninput="updateRadialPosMsgMask(event, 'x')">
                                <span class="numContUnit">%</span>
                            </div>
                            <div class="numCont">
                                <span class="numContDescr">y:</span>
                                <input class="numContInput" type="number" value="50" oninput="updateRadialPosMsgMask(event, 'y')">
                                <span class="numContUnit">%</span>
                            </div>
                        </div>
                    </div>
        ...
        document.querySelector(".colorMenuColMsgMask").insertBefore(colorMenu, document.querySelector(".colorMenuColMsgMask").children[indexPlace]);
    } else {
        indexPlace = index;

        var menuOpt = JSON.parse(JSON.stringify(settings.msgBackgroundMask[`${activeRole}`].value[indexPlace]));
        settings.msgBackgroundMask[`${activeRole}`].value.splice(indexPlace, 0, menuOpt);

        var colorMenuNode = document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[indexPlace].cloneNode(true);
        colorMenuNode.setAttribute("data-color-show", "false");
        colorMenuNode.setAttribute("data-set-show", "false");
        colorMenuNode.setAttribute("data-size-show", "false");
        document.querySelector(".colorMenuColMsgMask").insertBefore(colorMenuNode, document.querySelector(".colorMenuColMsgMask").children[indexPlace]);
    }
    
    for (var i = 0; i < document.querySelectorAll(".colorSliderMsgMaskCont").length; i++) {
        for (var j = 0; j < document.querySelectorAll(".colorSliderMsgMaskCont")[i].querySelectorAll(".colorSlider").length; j++) {
            document.querySelectorAll(".colorSliderMsgMaskCont")[i].querySelectorAll(".colorSlider")[j].setAttribute("onpointerdown", `openColorMsgMaskBox(${i}, ${j})`);
        }
    }
...
function removeGradientMsgMask(e) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode);
    if (i < activeMsgMaskGrad) {
        activeMsgMaskGrad -= 1;
    }
    settings.msgBackgroundMask[`${activeRole}`].value.splice(i, 1);
    document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[i].remove();
    for (let j = 0; j < document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu").length; j++) {
        for (let k = 0; k < document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[j].querySelectorAll(".colorSlider").length; k++) {
            document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[j].querySelectorAll(".colorSlider")[k].setAttribute("onpointerdown", `openColorMsgMaskBox(${j}, ${k})`)
        }
    }
...
function updateRadialTypeMsgMask(e, size) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode);
    settings.msgBackgroundMask[`${activeRole}`].value[i].size = size;
    for (let j = 0; j < settings.msgBackgroundMask[`${activeRole}`].value.length; j++) {
        if (settings.msgBackgroundMask[`${activeRole}`].value[j].type === "radial") {
            for (let k = 0; k < document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[j].querySelector(".gradRadialPosContMsgMask").querySelectorAll("button").length; k++) {
                document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[j].querySelector(".gradRadialPosContMsgMask").querySelectorAll("button")[k].setAttribute("data-active", "false");
            }
            e.target.setAttribute("data-active", "true");
        }
    }
...
function updateLinearRotMsgMask(e) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode.parentNode);
    settings.msgBackgroundMask[`${activeRole}`].value[i].rotate = parseInt(e.target.value);
    
    decompileMsgMask();
}

function updateRadialPosMsgMask(e, xy) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode.parentNode);
    if (xy === "x") {
        settings.msgBackgroundMask[`${activeRole}`].value[i].x = parseInt(e.target.value);
    } else if (xy === "y") {
        settings.msgBackgroundMask[`${activeRole}`].value[i].y = parseInt(e.target.value);
    }
    
    decompileMsgMask();
}

function moveColorSliderMsgMask(e, dir) {
    var i = Array.from(e.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(e.target.parentNode.parentNode.parentNode);
    if (dir === "up" && i > 0) {
        var elmnt = settings.msgBackgroundMask[`${activeRole}`].value[i];
        settings.msgBackgroundMask[`${activeRole}`].value.splice(i, 1);
        settings.msgBackgroundMask[`${activeRole}`].value.splice(i - 1, 0, elmnt);
        document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[i - 1].before(document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[i]);
        if (activeMsgMaskGrad === i) {
            activeMsgMaskGrad -= 1;
        } else if (activeMsgMaskGrad === i - 1) {
            activeMsgMaskGrad += 1;
        }
    } else if (dir === "down" && i < settings.msgBackgroundMask[`${activeRole}`].value.length - 1) {
        var elmnt = settings.msgBackgroundMask[`${activeRole}`].value[i];
        settings.msgBackgroundMask[`${activeRole}`].value.splice(i, 1);
        settings.msgBackgroundMask[`${activeRole}`].value.splice(i + 1, 0, elmnt);
        document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[i + 1].after(document.querySelector(".colorMenuColMsgMask").querySelectorAll(".colorMenu")[i]);
        if (activeMsgMaskGrad === i) {
            activeMsgMaskGrad += 1;
        } else if (activeMsgMaskGrad === i + 1) {
            activeMsgMaskGrad -= 1;
...
function duplicateColorSliderMsgMask() {
    activeMsgMaskGrad += 1;
    var i = Array.from(event.target.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode);
    var type = settings.msgBackgroundMask[`${activeRole}`].value[i].type;

    addGradientMsgMask(type, i);
}

function updateMsgMaskSizePos(cat, value) {
    var i = Array.from(event.target.parentNode.parentNode.parentNode.parentNode.children).indexOf(event.target.parentNode.parentNode.parentNode);
    if (cat === "posX") {
        if (value === "%") {
            settings.msgBackgroundMask[`${activeRole}`].value[i].posX.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.msgBackgroundMask[`${activeRole}`].value[i].posX.px = parseInt(event.target.value);
        }
    } else if (cat === "posY") {
        if (value === "%") {
            settings.msgBackgroundMask[`${activeRole}`].value[i].posY.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.msgBackgroundMask[`${activeRole}`].value[i].posY.px = parseInt(event.target.value);
        }
    } else if (cat === "width") {
        if (value === "%") {
            settings.msgBackgroundMask[`${activeRole}`].value[i].width.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.msgBackgroundMask[`${activeRole}`].value[i].width.px = parseInt(event.target.value);
        }
    } else if (cat === "height") {
        if (value === "%") {
            settings.msgBackgroundMask[`${activeRole}`].value[i].height.percent = parseInt(event.target.value);
        } else if (value === "px") {
            settings.msgBackgroundMask[`${activeRole}`].value[i].height.px = parseInt(event.target.value);
        }
    }
        ...
        if (settings.msgBorder[`${role}`].active === true) {
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                if (settings.msgBorder[`${role}`].value.top.color.type === "custom") {
                    ...
                    msg.style.setProperty("--msgBorderTop", `${settings.msgBorder[`${role}`].value.top.type} ${settings.msgBorder[`${role}`].value.top.width}px ${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.msgBorder[`${role}`].value.top.color.opacity)}`);
                }
                ...
                if (settings.msgBorder[`${role}`].value.right.color.type === "custom") {
                    ...
                    msg.style.setProperty("--msgBorderRight", `${settings.msgBorder[`${role}`].value.right.type} ${settings.msgBorder[`${role}`].value.right.width}px ${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.msgBorder[`${role}`].value.right.color.opacity)}`);
                }
                ...
                if (settings.msgBorder[`${role}`].value.bottom.color.type === "custom") {
                    ...
                    msg.style.setProperty("--msgBorderBottom", `${settings.msgBorder[`${role}`].value.bottom.type} ${settings.msgBorder[`${role}`].value.bottom.width}px ${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.msgBorder[`${role}`].value.bottom.color.opacity)}`);
                }
                ...
                if (settings.msgBorder[`${role}`].value.left.color.type === "custom") {
                    ...
                    msg.style.setProperty("--msgBorderLeft", `${settings.msgBorder[`${role}`].value.left.type} ${settings.msgBorder[`${role}`].value.left.width}px ${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.msgBorder[`${role}`].value.left.color.opacity)}`);
                }
            });
        }
    });
            ...
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                for (let i = 0; i < settings.msgShadow[`${role}`].value.length; i++) {
                    ...
                    if (settings.msgShadow[`${role}`].value[i].color.type === "custom") {
                        ...
                        msgShadow += `${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.msgShadow[`${role}`].value[i].color.opacity)}`;
                    }
...
function openShadowMsg(triggerAdd) {
    ...
    if (activeMsgShadow >= 0) {
        ...
        document.querySelector(".shadowSettingsMsg").querySelectorAll(".numContInput")[0].value = settings.msgShadow[`${activeRole}`].value[activeMsgShadow].x;
        document.querySelector(".shadowSettingsMsg").querySelectorAll(".numContInput")[1].value = settings.msgShadow[`${activeRole}`].value[activeMsgShadow].y;
        ...
        document.querySelector("#msgShadowOpacity").setAttribute("data-value", settings.msgShadow[`${activeRole}`].value[activeMsgShadow].color.opacity);
...
function updateShadowMsg(type, value) {
    ...
    settings.msgShadow[`${activeRole}`].value[activeMsgShadow].x    = parseInt(document.querySelector(".shadowSettingsMsg").querySelectorAll(".numContInput")[0].value);
    settings.msgShadow[`${activeRole}`].value[activeMsgShadow].y    = parseInt(document.querySelector(".shadowSettingsMsg").querySelectorAll(".numContInput")[1].value);
    ...
    document.querySelector("#msgShadowOpacity").setAttribute("data-value", document.querySelector("#msgShadowOpacity").value);
    ...
    for (let i = 0; i < settings.msgShadow[`${activeRole}`].value.length; i++) {
        var msgStyle = `${settings.msgShadow[`${activeRole}`].value[i].x}px ${settings.msgShadow[`${activeRole}`].value[i].y}px ${settings.msgShadow[`${activeRole}`].value[i].blur}px ${settings.msgShadow[`${activeRole}`].value[i].expand}px ${settings.msgShadow[`${activeRole}`].value[i].color.color}${dec2hex(settings.msgShadow[`${activeRole}`].value[i].color.opacity)}`;
...
function updateMsgSizingH(type) {
    if (type === "stretch") {
        event.target.parentNode.querySelectorAll("button")[0].setAttribute("data-active", "true");
        event.target.parentNode.querySelectorAll("button")[1].setAttribute("data-active", "false");
        event.target.parentNode.querySelectorAll("button")[2].setAttribute("data-active", "false");
    ...
    settings.msgPosition[`${activeRole}`].value.sizingH = type;

    decompileMsgSizing();
}

function updateMsgSizingV(type) {
    if (type === "stretch") {
        event.target.parentNode.querySelectorAll("button")[0].setAttribute("data-active", "true");
        event.target.parentNode.querySelectorAll("button")[1].setAttribute("data-active", "false");
        event.target.parentNode.querySelectorAll("button")[2].setAttribute("data-active", "false");
    ...
    settings.msgPosition[`${activeRole}`].value.sizingV = type;
        ...
        if (settings.msgFont[`${role}`].active === true) {
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                if (settings.msgFont[`${role}`].value.color.type === "custom") {
                    ...
                    msg.style.setProperty("--msgFontColor", `${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.msgFont[`${role}`].value.color.opacity)}`);
                }
...
function updateMsgFont(type, value) {
    ...
    if (type === "style") {
        ...
        settings.msgFont[`${activeRole}`].value.hAlign = event.target.getAttribute("data-value");
    } else if (type === "autoheight") {
        if (event.target.getAttribute("data-active") === "true") {
            event.target.setAttribute("data-active", "false");
            ...
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                for (let i = 0; i < settings.msgFontShadow[`${role}`].value.length; i++) {
                    ...
                    if (settings.msgFontShadow[`${role}`].value[i].color.type === "custom") {
                        ...
                        msgFontShadow += `${msg.parentNode.parentNode.getAttribute("data-twitch")}${dec2hex(settings.msgFontShadow[`${role}`].value[i].color.opacity)}`;
                    }
...
function openFontShadowMsg(triggerAdd) {
    ...
    if (activeMsgFontShadow >= 0) {
        ...
        document.querySelector(".fontShadowSettingsMsg").querySelectorAll(".numContInput")[0].value = settings.msgFontShadow[`${activeRole}`].value[activeMsgFontShadow].x;
        document.querySelector(".fontShadowSettingsMsg").querySelectorAll(".numContInput")[1].value = settings.msgFontShadow[`${activeRole}`].value[activeMsgFontShadow].y;
        ...
        document.querySelector("#msgFontShadowOpacity").setAttribute("data-value", settings.msgFontShadow[`${activeRole}`].value[activeMsgFontShadow].color.opacity);
...
function updateFontShadowMsg(type, value) {
    ...
    settings.msgFontShadow[`${activeRole}`].value[activeMsgFontShadow].x = parseInt(document.querySelector(".fontShadowSettingsMsg").querySelectorAll(".numContInput")[0].value);
    settings.msgFontShadow[`${activeRole}`].value[activeMsgFontShadow].y = parseInt(document.querySelector(".fontShadowSettingsMsg").querySelectorAll(".numContInput")[1].value);
    ...
    for (let i = 0; i < settings.msgFontShadow[`${activeRole}`].value.length; i++) {
        var msgStyle = `${settings.msgFontShadow[`${activeRole}`].value[i].x}px ${settings.msgFontShadow[`${activeRole}`].value[i].y}px ${settings.msgFontShadow[`${activeRole}`].value[i].blur}px ${settings.msgFontShadow[`${activeRole}`].value[i].color.color}${dec2hex(settings.msgFontShadow[`${activeRole}`].value[i].color.opacity)}`;
    ...
    roleList.forEach(role => {
        if (settings.msgContent[`${role}`].active === true) {
            document.querySelectorAll(`.cbxW.${role}`).forEach(cbxW => {
                ...
                msgContentFinal = msgContentFinal.replace(/{usn}/g, msg.parentNode.parentNode.getAttribute("data-username"));

                msgContentFinal = msgContentFinal.replace(/{msg}/g, msg.parentNode.parentNode.getAttribute("data-message"));

                msgContentFinal = msgContentFinal.replace(/{streamer}/g, msg.parentNode.parentNode.getAttribute("data-streamer"));
                
                msgContentFinal = msgContentFinal.replace(/{h}/g, msg.parentNode.parentNode.getAttribute("data-h"));
                
                msgContentFinal = msgContentFinal.replace(/{hh}/g, msg.parentNode.parentNode.getAttribute("data-hh"));

                msgContentFinal = msgContentFinal.replace(/{h12}/g, msg.parentNode.parentNode.getAttribute("data-h12"));

                msgContentFinal = msgContentFinal.replace(/{hh12}/g, msg.parentNode.parentNode.getAttribute("data-hh12"));

                msgContentFinal = msgContentFinal.replace(/{apm}/g, msg.parentNode.parentNode.getAttribute("data-apm"));
                
                msgContentFinal = msgContentFinal.replace(/{m}/g, msg.parentNode.parentNode.getAttribute("data-m"));
                
                msgContentFinal = msgContentFinal.replace(/{mm}/g, msg.parentNode.parentNode.getAttribute("data-mm"));
                
                msgContentFinal = msgContentFinal.replace(/{s}/g, msg.parentNode.parentNode.getAttribute("data-s"));
...
function decompileOthHyphens() {
    console.log("Test");
    document.querySelectorAll(`.cbxW`).forEach(cbxW => {
...
function activateOthHyphens(activate) {
    if (activate === true) {
        document.querySelectorAll(".othHyphensBtn")[0].setAttribute("data-active", "false");
        document.querySelectorAll(".othHyphensBtn")[1].setAttribute("data-active", "true");
        document.querySelector(".scrollList").setAttribute("data-show", "true");
        settings.othHyphens.active = true;

    } else if (activate === false) {
        document.querySelectorAll(".othHyphensBtn")[0].setAttribute("data-active", "true");
        document.querySelectorAll(".othHyphensBtn")[1].setAttribute("data-active", "false");
        document.querySelector(".scrollList").setAttribute("data-show", "false");
        settings.othHyphens.active = false;
    }

    decompileOthHyphens();
}

function updateOthHyphensLang(lang) {
    settings.othHyphens.lang = lang;
...
function updateAnmShow(i) {
    if (settings.anmShow[`${activeRole}`].value[i].type === "curve") {
        if (document.querySelector("#anmShowCurve").value.length > 0) {
            settings.anmShow[`${activeRole}`].value[i].value = document.querySelector("#anmShowCurve").value;
        } else {
            settings.anmShow[`${activeRole}`].value[i].value = "cubic-bezier(.5,0,.5,1)";
        }
    } else {
...
function updateAnmHide(i) {
    if (settings.anmHide[`${activeRole}`].value[i].type === "curve") {
        if (document.querySelector("#anmHideCurve").value.length > 0) {
            settings.anmHide[`${activeRole}`].value[i].value = document.querySelector("#anmHideCurve").value;
        } else {
            settings.anmHide[`${activeRole}`].value[i].value = "cubic-bezier(.5,0,.5,1)";
        }
    } else {
...
function saveSettings() {
    ...
    for (var i = 0; i < settings.cbxBackground.user.value.length; i++) {
        if (settings.cbxBackground.user.value[i].type === "solid") {
            cbxBackgroundResUser += `{
                type: "solid",
                name: "${settings.cbxBackground.user.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.cbxBackground.user.value[i].posX.percent},
                    px: ${settings.cbxBackground.user.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.cbxBackground.user.value[i].posY.percent},
                    px: ${settings.cbxBackground.user.value[i].posY.px}
                },
                width: {
                    percent: ${settings.cbxBackground.user.value[i].width.percent},
                    px: ${settings.cbxBackground.user.value[i].width.px},
                },
                height: {
                    percent: ${settings.cbxBackground.user.value[i].height.percent},
                    px: ${settings.cbxBackground.user.value[i].height.px}
                }
            }`;
            ...
            cbxBackgroundResUser += `{
                ...
                x: ${settings.cbxBackground.user.value[i].x},
                y: ${settings.cbxBackground.user.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    for (var i = 0; i < settings.cbxBackground.sub.value.length; i++) {
        if (settings.cbxBackground.sub.value[i].type === "solid") {
            cbxBackgroundResSub += `{
                type: "solid",
                name: "${settings.cbxBackground.sub.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.cbxBackground.sub.value[i].posX.percent},
                    px: ${settings.cbxBackground.sub.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.cbxBackground.sub.value[i].posY.percent},
                    px: ${settings.cbxBackground.sub.value[i].posY.px}
                },
                width: {
                    percent: ${settings.cbxBackground.sub.value[i].width.percent},
                    px: ${settings.cbxBackground.sub.value[i].width.px},
                },
                height: {
                    percent: ${settings.cbxBackground.sub.value[i].height.percent},
                    px: ${settings.cbxBackground.sub.value[i].height.px}
                }
            }`;
            ...
            cbxBackgroundResSub += `{
                ...
                x: ${settings.cbxBackground.sub.value[i].x},
                y: ${settings.cbxBackground.sub.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    for (var i = 0; i < settings.cbxBackground.vip.value.length; i++) {
        if (settings.cbxBackground.vip.value[i].type === "solid") {
            cbxBackgroundResVip += `{
                type: "solid",
                name: "${settings.cbxBackground.vip.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.cbxBackground.vip.value[i].posX.percent},
                    px: ${settings.cbxBackground.vip.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.cbxBackground.vip.value[i].posY.percent},
                    px: ${settings.cbxBackground.vip.value[i].posY.px}
                },
                width: {
                    percent: ${settings.cbxBackground.vip.value[i].width.percent},
                    px: ${settings.cbxBackground.vip.value[i].width.px},
                },
                height: {
                    percent: ${settings.cbxBackground.vip.value[i].height.percent},
                    px: ${settings.cbxBackground.vip.value[i].height.px}
                }
            }`;
            ...
            cbxBackgroundResVip += `{
                ...
                x: ${settings.cbxBackground.vip.value[i].x},
                y: ${settings.cbxBackground.vip.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    for (var i = 0; i < settings.cbxBackground.mod.value.length; i++) {
        if (settings.cbxBackground.mod.value[i].type === "solid") {
            cbxBackgroundResMod += `{
                type: "solid",
                name: "${settings.cbxBackground.mod.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.cbxBackground.mod.value[i].posX.percent},
                    px: ${settings.cbxBackground.mod.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.cbxBackground.mod.value[i].posY.percent},
                    px: ${settings.cbxBackground.mod.value[i].posY.px}
                },
                width: {
                    percent: ${settings.cbxBackground.mod.value[i].width.percent},
                    px: ${settings.cbxBackground.mod.value[i].width.px},
                },
                height: {
                    percent: ${settings.cbxBackground.mod.value[i].height.percent},
                    px: ${settings.cbxBackground.mod.value[i].height.px}
                }
            }`;
            ...
            cbxBackgroundResMod += `{
                ...
                x: ${settings.cbxBackground.mod.value[i].x},
                y: ${settings.cbxBackground.mod.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    var cbxBackgroundMaskResUser = "";

    for (var i = 0; i < settings.cbxBackgroundMask.user.value.length; i++) {
        if (settings.cbxBackgroundMask.user.value[i].type === "solid") {
            cbxBackgroundMaskResUser += `{
                type: "solid",
                name: "${settings.cbxBackgroundMask.user.value[i].name}",
                colors: [
                    {
                        position: ${settings.cbxBackgroundMask.user.value[i].colors[0].position},
                        opacity: ${settings.cbxBackgroundMask.user.value[i].colors[0].opacity},
                        count: ${settings.cbxBackgroundMask.user.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.cbxBackgroundMask.user.value[i].posX.percent},
                    px: ${settings.cbxBackgroundMask.user.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.cbxBackgroundMask.user.value[i].posY.percent},
                    px: ${settings.cbxBackgroundMask.user.value[i].posY.px}
                },
                width: {
                    percent: ${settings.cbxBackgroundMask.user.value[i].width.percent},
                    px: ${settings.cbxBackgroundMask.user.value[i].width.px},
                },
                height: {
                    percent: ${settings.cbxBackgroundMask.user.value[i].height.percent},
                    px: ${settings.cbxBackgroundMask.user.value[i].height.px}
                }
            }`;
        } else if (settings.cbxBackgroundMask.user.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.cbxBackgroundMask.user.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.cbxBackgroundMask.user.value[i].colors[j].position},
                    opacity: ${settings.cbxBackgroundMask.user.value[i].colors[j].opacity},
                    count: ${settings.cbxBackgroundMask.user.value[i].colors[j].count}
                }`;

                if (j < settings.cbxBackgroundMask.user.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            cbxBackgroundMaskResUser += `{
                type: "linear",
                name: "${settings.cbxBackgroundMask.user.value[i].name}",
                rotate: ${settings.cbxBackgroundMask.user.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.cbxBackgroundMask.user.value[i].type === "radial") {
            var colorList = "";
            ...
            cbxBackgroundMaskResUser += `{
                type: "radial",
                name: "${settings.cbxBackgroundMask.user.value[i].name}",
                size: "${settings.cbxBackgroundMask.user.value[i].size}",
                x: ${settings.cbxBackgroundMask.user.value[i].x},
                y: ${settings.cbxBackgroundMask.user.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.cbxBackgroundMask.user.value.length - 1) {
            cbxBackgroundMaskResUser += `, `;
        }
    }

    var cbxBackgroundMaskResSub = "";

    for (var i = 0; i < settings.cbxBackgroundMask.sub.value.length; i++) {
        if (settings.cbxBackgroundMask.sub.value[i].type === "solid") {
            cbxBackgroundMaskResSub += `{
                type: "solid",
                name: "${settings.cbxBackgroundMask.sub.value[i].name}",
                colors: [
                    {
                        position: ${settings.cbxBackgroundMask.sub.value[i].colors[0].position},
                        opacity: ${settings.cbxBackgroundMask.sub.value[i].colors[0].opacity},
                        count: ${settings.cbxBackgroundMask.sub.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.cbxBackgroundMask.sub.value[i].posX.percent},
                    px: ${settings.cbxBackgroundMask.sub.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.cbxBackgroundMask.sub.value[i].posY.percent},
                    px: ${settings.cbxBackgroundMask.sub.value[i].posY.px}
                },
                width: {
                    percent: ${settings.cbxBackgroundMask.sub.value[i].width.percent},
                    px: ${settings.cbxBackgroundMask.sub.value[i].width.px},
                },
                height: {
                    percent: ${settings.cbxBackgroundMask.sub.value[i].height.percent},
                    px: ${settings.cbxBackgroundMask.sub.value[i].height.px}
                }
            }`;
        } else if (settings.cbxBackgroundMask.sub.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.cbxBackgroundMask.sub.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.cbxBackgroundMask.sub.value[i].colors[j].position},
                    opacity: ${settings.cbxBackgroundMask.sub.value[i].colors[j].opacity},
                    count: ${settings.cbxBackgroundMask.sub.value[i].colors[j].count}
                }`;

                if (j < settings.cbxBackgroundMask.sub.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            cbxBackgroundMaskResSub += `{
                type: "linear",
                name: "${settings.cbxBackgroundMask.sub.value[i].name}",
                rotate: ${settings.cbxBackgroundMask.sub.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.cbxBackgroundMask.sub.value[i].type === "radial") {
            var colorList = "";
            ...
            cbxBackgroundMaskResSub += `{
                type: "radial",
                name: "${settings.cbxBackgroundMask.sub.value[i].name}",
                size: "${settings.cbxBackgroundMask.sub.value[i].size}",
                x: ${settings.cbxBackgroundMask.sub.value[i].x},
                y: ${settings.cbxBackgroundMask.sub.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.cbxBackgroundMask.sub.value.length - 1) {
            cbxBackgroundMaskResSub += `, `;
        }
    }

    var cbxBackgroundMaskResVip = "";

    for (var i = 0; i < settings.cbxBackgroundMask.vip.value.length; i++) {
        if (settings.cbxBackgroundMask.vip.value[i].type === "solid") {
            cbxBackgroundMaskResVip += `{
                type: "solid",
                name: "${settings.cbxBackgroundMask.vip.value[i].name}",
                colors: [
                    {
                        position: ${settings.cbxBackgroundMask.vip.value[i].colors[0].position},
                        opacity: ${settings.cbxBackgroundMask.vip.value[i].colors[0].opacity},
                        count: ${settings.cbxBackgroundMask.vip.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.cbxBackgroundMask.vip.value[i].posX.percent},
                    px: ${settings.cbxBackgroundMask.vip.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.cbxBackgroundMask.vip.value[i].posY.percent},
                    px: ${settings.cbxBackgroundMask.vip.value[i].posY.px}
                },
                width: {
                    percent: ${settings.cbxBackgroundMask.vip.value[i].width.percent},
                    px: ${settings.cbxBackgroundMask.vip.value[i].width.px},
                },
                height: {
                    percent: ${settings.cbxBackgroundMask.vip.value[i].height.percent},
                    px: ${settings.cbxBackgroundMask.vip.value[i].height.px}
                }
            }`;
        } else if (settings.cbxBackgroundMask.vip.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.cbxBackgroundMask.vip.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.cbxBackgroundMask.vip.value[i].colors[j].position},
                    opacity: ${settings.cbxBackgroundMask.vip.value[i].colors[j].opacity},
                    count: ${settings.cbxBackgroundMask.vip.value[i].colors[j].count}
                }`;

                if (j < settings.cbxBackgroundMask.vip.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            cbxBackgroundMaskResVip += `{
                type: "linear",
                name: "${settings.cbxBackgroundMask.vip.value[i].name}",
                rotate: ${settings.cbxBackgroundMask.vip.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.cbxBackgroundMask.vip.value[i].type === "radial") {
            var colorList = "";
            ...
            cbxBackgroundMaskResVip += `{
                type: "radial",
                name: "${settings.cbxBackgroundMask.vip.value[i].name}",
                size: "${settings.cbxBackgroundMask.vip.value[i].size}",
                x: ${settings.cbxBackgroundMask.vip.value[i].x},
                y: ${settings.cbxBackgroundMask.vip.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.cbxBackgroundMask.vip.value.length - 1) {
            cbxBackgroundMaskResVip += `, `;
        }
    }

    var cbxBackgroundMaskResMod = "";

    for (var i = 0; i < settings.cbxBackgroundMask.mod.value.length; i++) {
        if (settings.cbxBackgroundMask.mod.value[i].type === "solid") {
            cbxBackgroundMaskResMod += `{
                type: "solid",
                name: "${settings.cbxBackgroundMask.mod.value[i].name}",
                colors: [
                    {
                        position: ${settings.cbxBackgroundMask.mod.value[i].colors[0].position},
                        opacity: ${settings.cbxBackgroundMask.mod.value[i].colors[0].opacity},
                        count: ${settings.cbxBackgroundMask.mod.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.cbxBackgroundMask.mod.value[i].posX.percent},
                    px: ${settings.cbxBackgroundMask.mod.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.cbxBackgroundMask.mod.value[i].posY.percent},
                    px: ${settings.cbxBackgroundMask.mod.value[i].posY.px}
                },
                width: {
                    percent: ${settings.cbxBackgroundMask.mod.value[i].width.percent},
                    px: ${settings.cbxBackgroundMask.mod.value[i].width.px},
                },
                height: {
                    percent: ${settings.cbxBackgroundMask.mod.value[i].height.percent},
                    px: ${settings.cbxBackgroundMask.mod.value[i].height.px}
                }
            }`;
        } else if (settings.cbxBackgroundMask.mod.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.cbxBackgroundMask.mod.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.cbxBackgroundMask.mod.value[i].colors[j].position},
                    opacity: ${settings.cbxBackgroundMask.mod.value[i].colors[j].opacity},
                    count: ${settings.cbxBackgroundMask.mod.value[i].colors[j].count}
                }`;

                if (j < settings.cbxBackgroundMask.mod.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            cbxBackgroundMaskResMod += `{
                type: "linear",
                name: "${settings.cbxBackgroundMask.mod.value[i].name}",
                rotate: ${settings.cbxBackgroundMask.mod.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.cbxBackgroundMask.mod.value[i].type === "radial") {
            var colorList = "";
            ...
            cbxBackgroundMaskResMod += `{
                type: "radial",
                name: "${settings.cbxBackgroundMask.mod.value[i].name}",
                size: "${settings.cbxBackgroundMask.mod.value[i].size}",
                x: ${settings.cbxBackgroundMask.mod.value[i].x},
                y: ${settings.cbxBackgroundMask.mod.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.cbxBackgroundMask.mod.value.length - 1) {
            cbxBackgroundMaskResMod += `, `;
        }
    }
    ...
    for (var i = 0; i < settings.cbxShadow.user.value.length; i++) {
        cbxShadowResUser += `{
        x: ${settings.cbxShadow.user.value[i].x},
        y: ${settings.cbxShadow.user.value[i].y},
    ...
    for (var i = 0; i < settings.cbxShadow.sub.value.length; i++) {
        cbxShadowResSub += `{
        x: ${settings.cbxShadow.sub.value[i].x},
        y: ${settings.cbxShadow.sub.value[i].y},
    ...
    for (var i = 0; i < settings.cbxShadow.vip.value.length; i++) {
        cbxShadowResVip += `{
        x: ${settings.cbxShadow.vip.value[i].x},
        y: ${settings.cbxShadow.vip.value[i].y},
    ...
    for (var i = 0; i < settings.cbxShadow.mod.value.length; i++) {
        cbxShadowResMod += `{
        x: ${settings.cbxShadow.mod.value[i].x},
        y: ${settings.cbxShadow.mod.value[i].y},
    ...
    for (var i = 0; i < settings.usnBackground.user.value.length; i++) {
        if (settings.usnBackground.user.value[i].type === "solid") {
            usnBackgroundResUser += `{
                type: "solid",
                name: "${settings.usnBackground.user.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.usnBackground.user.value[i].posX.percent},
                    px: ${settings.usnBackground.user.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.usnBackground.user.value[i].posY.percent},
                    px: ${settings.usnBackground.user.value[i].posY.px}
                },
                width: {
                    percent: ${settings.usnBackground.user.value[i].width.percent},
                    px: ${settings.usnBackground.user.value[i].width.px},
                },
                height: {
                    percent: ${settings.usnBackground.user.value[i].height.percent},
                    px: ${settings.usnBackground.user.value[i].height.px}
                }
            }`;
            ...
            usnBackgroundResUser += `{
                ...
                x: ${settings.usnBackground.user.value[i].x},
                y: ${settings.usnBackground.user.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    for (var i = 0; i < settings.usnBackground.sub.value.length; i++) {
        if (settings.usnBackground.sub.value[i].type === "solid") {
            usnBackgroundResSub += `{
                type: "solid",
                name: "${settings.usnBackground.sub.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.usnBackground.sub.value[i].posX.percent},
                    px: ${settings.usnBackground.sub.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.usnBackground.sub.value[i].posY.percent},
                    px: ${settings.usnBackground.sub.value[i].posY.px}
                },
                width: {
                    percent: ${settings.usnBackground.sub.value[i].width.percent},
                    px: ${settings.usnBackground.sub.value[i].width.px},
                },
                height: {
                    percent: ${settings.usnBackground.sub.value[i].height.percent},
                    px: ${settings.usnBackground.sub.value[i].height.px}
                }
            }`;
            ...
            usnBackgroundResSub += `{
                ...
                x: ${settings.usnBackground.sub.value[i].x},
                y: ${settings.usnBackground.sub.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    for (var i = 0; i < settings.usnBackground.vip.value.length; i++) {
        if (settings.usnBackground.vip.value[i].type === "solid") {
            usnBackgroundResVip += `{
                type: "solid",
                name: "${settings.usnBackground.vip.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.usnBackground.vip.value[i].posX.percent},
                    px: ${settings.usnBackground.vip.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.usnBackground.vip.value[i].posY.percent},
                    px: ${settings.usnBackground.vip.value[i].posY.px}
                },
                width: {
                    percent: ${settings.usnBackground.vip.value[i].width.percent},
                    px: ${settings.usnBackground.vip.value[i].width.px},
                },
                height: {
                    percent: ${settings.usnBackground.vip.value[i].height.percent},
                    px: ${settings.usnBackground.vip.value[i].height.px}
                }
            }`;
            ...
            usnBackgroundResVip += `{
                ...
                x: ${settings.usnBackground.vip.value[i].x},
                y: ${settings.usnBackground.vip.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    for (var i = 0; i < settings.usnBackground.mod.value.length; i++) {
        if (settings.usnBackground.mod.value[i].type === "solid") {
            usnBackgroundResMod += `{
                type: "solid",
                name: "${settings.usnBackground.mod.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.usnBackground.mod.value[i].posX.percent},
                    px: ${settings.usnBackground.mod.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.usnBackground.mod.value[i].posY.percent},
                    px: ${settings.usnBackground.mod.value[i].posY.px}
                },
                width: {
                    percent: ${settings.usnBackground.mod.value[i].width.percent},
                    px: ${settings.usnBackground.mod.value[i].width.px},
                },
                height: {
                    percent: ${settings.usnBackground.mod.value[i].height.percent},
                    px: ${settings.usnBackground.mod.value[i].height.px}
                }
            }`;
            ...
            usnBackgroundResMod += `{
                ...
                x: ${settings.usnBackground.mod.value[i].x},
                y: ${settings.usnBackground.mod.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    var usnBackgroundMaskResUser = "";

    for (var i = 0; i < settings.usnBackgroundMask.user.value.length; i++) {
        if (settings.usnBackgroundMask.user.value[i].type === "solid") {
            usnBackgroundMaskResUser += `{
                type: "solid",
                name: "${settings.usnBackgroundMask.user.value[i].name}",
                colors: [
                    {
                        position: ${settings.usnBackgroundMask.user.value[i].colors[0].position},
                        opacity: ${settings.usnBackgroundMask.user.value[i].colors[0].opacity},
                        count: ${settings.usnBackgroundMask.user.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.usnBackgroundMask.user.value[i].posX.percent},
                    px: ${settings.usnBackgroundMask.user.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.usnBackgroundMask.user.value[i].posY.percent},
                    px: ${settings.usnBackgroundMask.user.value[i].posY.px}
                },
                width: {
                    percent: ${settings.usnBackgroundMask.user.value[i].width.percent},
                    px: ${settings.usnBackgroundMask.user.value[i].width.px},
                },
                height: {
                    percent: ${settings.usnBackgroundMask.user.value[i].height.percent},
                    px: ${settings.usnBackgroundMask.user.value[i].height.px}
                }
            }`;
        } else if (settings.usnBackgroundMask.user.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.usnBackgroundMask.user.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.usnBackgroundMask.user.value[i].colors[j].position},
                    opacity: ${settings.usnBackgroundMask.user.value[i].colors[j].opacity},
                    count: ${settings.usnBackgroundMask.user.value[i].colors[j].count}
                }`;

                if (j < settings.usnBackgroundMask.user.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            usnBackgroundMaskResUser += `{
                type: "linear",
                name: "${settings.usnBackgroundMask.user.value[i].name}",
                rotate: ${settings.usnBackgroundMask.user.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.usnBackgroundMask.user.value[i].type === "radial") {
            var colorList = "";
            ...
            usnBackgroundMaskResUser += `{
                type: "radial",
                name: "${settings.usnBackgroundMask.user.value[i].name}",
                size: "${settings.usnBackgroundMask.user.value[i].size}",
                x: ${settings.usnBackgroundMask.user.value[i].x},
                y: ${settings.usnBackgroundMask.user.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.usnBackgroundMask.user.value.length - 1) {
            usnBackgroundMaskResUser += `, `;
        }
    }

    var usnBackgroundMaskResSub = "";

    for (var i = 0; i < settings.usnBackgroundMask.sub.value.length; i++) {
        if (settings.usnBackgroundMask.sub.value[i].type === "solid") {
            usnBackgroundMaskResSub += `{
                type: "solid",
                name: "${settings.usnBackgroundMask.sub.value[i].name}",
                colors: [
                    {
                        position: ${settings.usnBackgroundMask.sub.value[i].colors[0].position},
                        opacity: ${settings.usnBackgroundMask.sub.value[i].colors[0].opacity},
                        count: ${settings.usnBackgroundMask.sub.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.usnBackgroundMask.sub.value[i].posX.percent},
                    px: ${settings.usnBackgroundMask.sub.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.usnBackgroundMask.sub.value[i].posY.percent},
                    px: ${settings.usnBackgroundMask.sub.value[i].posY.px}
                },
                width: {
                    percent: ${settings.usnBackgroundMask.sub.value[i].width.percent},
                    px: ${settings.usnBackgroundMask.sub.value[i].width.px},
                },
                height: {
                    percent: ${settings.usnBackgroundMask.sub.value[i].height.percent},
                    px: ${settings.usnBackgroundMask.sub.value[i].height.px}
                }
            }`;
        } else if (settings.usnBackgroundMask.sub.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.usnBackgroundMask.sub.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.usnBackgroundMask.sub.value[i].colors[j].position},
                    opacity: ${settings.usnBackgroundMask.sub.value[i].colors[j].opacity},
                    count: ${settings.usnBackgroundMask.sub.value[i].colors[j].count}
                }`;

                if (j < settings.usnBackgroundMask.sub.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            usnBackgroundMaskResSub += `{
                type: "linear",
                name: "${settings.usnBackgroundMask.sub.value[i].name}",
                rotate: ${settings.usnBackgroundMask.sub.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.usnBackgroundMask.sub.value[i].type === "radial") {
            var colorList = "";
            ...
            usnBackgroundMaskResSub += `{
                type: "radial",
                name: "${settings.usnBackgroundMask.sub.value[i].name}",
                size: "${settings.usnBackgroundMask.sub.value[i].size}",
                x: ${settings.usnBackgroundMask.sub.value[i].x},
                y: ${settings.usnBackgroundMask.sub.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.usnBackgroundMask.sub.value.length - 1) {
            usnBackgroundMaskResSub += `, `;
        }
    }

    var usnBackgroundMaskResVip = "";

    for (var i = 0; i < settings.usnBackgroundMask.vip.value.length; i++) {
        if (settings.usnBackgroundMask.vip.value[i].type === "solid") {
            usnBackgroundMaskResVip += `{
                type: "solid",
                name: "${settings.usnBackgroundMask.vip.value[i].name}",
                colors: [
                    {
                        position: ${settings.usnBackgroundMask.vip.value[i].colors[0].position},
                        opacity: ${settings.usnBackgroundMask.vip.value[i].colors[0].opacity},
                        count: ${settings.usnBackgroundMask.vip.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.usnBackgroundMask.vip.value[i].posX.percent},
                    px: ${settings.usnBackgroundMask.vip.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.usnBackgroundMask.vip.value[i].posY.percent},
                    px: ${settings.usnBackgroundMask.vip.value[i].posY.px}
                },
                width: {
                    percent: ${settings.usnBackgroundMask.vip.value[i].width.percent},
                    px: ${settings.usnBackgroundMask.vip.value[i].width.px},
                },
                height: {
                    percent: ${settings.usnBackgroundMask.vip.value[i].height.percent},
                    px: ${settings.usnBackgroundMask.vip.value[i].height.px}
                }
            }`;
        } else if (settings.usnBackgroundMask.vip.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.usnBackgroundMask.vip.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.usnBackgroundMask.vip.value[i].colors[j].position},
                    opacity: ${settings.usnBackgroundMask.vip.value[i].colors[j].opacity},
                    count: ${settings.usnBackgroundMask.vip.value[i].colors[j].count}
                }`;

                if (j < settings.usnBackgroundMask.vip.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            usnBackgroundMaskResVip += `{
                type: "linear",
                name: "${settings.usnBackgroundMask.vip.value[i].name}",
                rotate: ${settings.usnBackgroundMask.vip.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.usnBackgroundMask.vip.value[i].type === "radial") {
            var colorList = "";
            ...
            usnBackgroundMaskResVip += `{
                type: "radial",
                name: "${settings.usnBackgroundMask.vip.value[i].name}",
                size: "${settings.usnBackgroundMask.vip.value[i].size}",
                x: ${settings.usnBackgroundMask.vip.value[i].x},
                y: ${settings.usnBackgroundMask.vip.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.usnBackgroundMask.vip.value.length - 1) {
            usnBackgroundMaskResVip += `, `;
        }
    }

    var usnBackgroundMaskResMod = "";

    for (var i = 0; i < settings.usnBackgroundMask.mod.value.length; i++) {
        if (settings.usnBackgroundMask.mod.value[i].type === "solid") {
            usnBackgroundMaskResMod += `{
                type: "solid",
                name: "${settings.usnBackgroundMask.mod.value[i].name}",
                colors: [
                    {
                        position: ${settings.usnBackgroundMask.mod.value[i].colors[0].position},
                        opacity: ${settings.usnBackgroundMask.mod.value[i].colors[0].opacity},
                        count: ${settings.usnBackgroundMask.mod.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.usnBackgroundMask.mod.value[i].posX.percent},
                    px: ${settings.usnBackgroundMask.mod.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.usnBackgroundMask.mod.value[i].posY.percent},
                    px: ${settings.usnBackgroundMask.mod.value[i].posY.px}
                },
                width: {
                    percent: ${settings.usnBackgroundMask.mod.value[i].width.percent},
                    px: ${settings.usnBackgroundMask.mod.value[i].width.px},
                },
                height: {
                    percent: ${settings.usnBackgroundMask.mod.value[i].height.percent},
                    px: ${settings.usnBackgroundMask.mod.value[i].height.px}
                }
            }`;
        } else if (settings.usnBackgroundMask.mod.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.usnBackgroundMask.mod.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.usnBackgroundMask.mod.value[i].colors[j].position},
                    opacity: ${settings.usnBackgroundMask.mod.value[i].colors[j].opacity},
                    count: ${settings.usnBackgroundMask.mod.value[i].colors[j].count}
                }`;

                if (j < settings.usnBackgroundMask.mod.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            usnBackgroundMaskResMod += `{
                type: "linear",
                name: "${settings.usnBackgroundMask.mod.value[i].name}",
                rotate: ${settings.usnBackgroundMask.mod.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.usnBackgroundMask.mod.value[i].type === "radial") {
            var colorList = "";
            ...
            usnBackgroundMaskResMod += `{
                type: "radial",
                name: "${settings.usnBackgroundMask.mod.value[i].name}",
                size: "${settings.usnBackgroundMask.mod.value[i].size}",
                x: ${settings.usnBackgroundMask.mod.value[i].x},
                y: ${settings.usnBackgroundMask.mod.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.usnBackgroundMask.mod.value.length - 1) {
            usnBackgroundMaskResMod += `, `;
        }
    }
    ...
    for (var i = 0; i < settings.usnShadow.user.value.length; i++) {
        usnShadowResUser += `{
        x: ${settings.usnShadow.user.value[i].x},
        y: ${settings.usnShadow.user.value[i].y},
    ...
    for (var i = 0; i < settings.usnShadow.sub.value.length; i++) {
        usnShadowResSub += `{
        x: ${settings.usnShadow.sub.value[i].x},
        y: ${settings.usnShadow.sub.value[i].y},
    ...
    for (var i = 0; i < settings.usnShadow.vip.value.length; i++) {
        usnShadowResVip += `{
        x: ${settings.usnShadow.vip.value[i].x},
        y: ${settings.usnShadow.vip.value[i].y},
    ...
    for (var i = 0; i < settings.usnShadow.mod.value.length; i++) {
        usnShadowResMod += `{
        x: ${settings.usnShadow.mod.value[i].x},
        y: ${settings.usnShadow.mod.value[i].y},
    ...
    for (var i = 0; i < settings.usnFontShadow.user.value.length; i++) {
        ...
        x: ${settings.usnFontShadow.user.value[i].x},
        y: ${settings.usnFontShadow.user.value[i].y},
    ...
    for (var i = 0; i < settings.usnFontShadow.sub.value.length; i++) {
        usnFontShadowResSub += `{
        x: ${settings.usnFontShadow.sub.value[i].x},
        y: ${settings.usnFontShadow.sub.value[i].y},
    ...
    for (var i = 0; i < settings.usnFontShadow.vip.value.length; i++) {
        usnFontShadowResVip += `{
        x: ${settings.usnFontShadow.vip.value[i].x},
        y: ${settings.usnFontShadow.vip.value[i].y},
    ...
    for (var i = 0; i < settings.usnFontShadow.mod.value.length; i++) {
        usnFontShadowResMod += `{
        x: ${settings.usnFontShadow.mod.value[i].x},
        y: ${settings.usnFontShadow.mod.value[i].y},
    ...
    for (var i = 0; i < settings.msgBackground.user.value.length; i++) {
        if (settings.msgBackground.user.value[i].type === "solid") {
            msgBackgroundResUser += `{
                type: "solid",
                name: "${settings.msgBackground.user.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.msgBackground.user.value[i].posX.percent},
                    px: ${settings.msgBackground.user.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.msgBackground.user.value[i].posY.percent},
                    px: ${settings.msgBackground.user.value[i].posY.px}
                },
                width: {
                    percent: ${settings.msgBackground.user.value[i].width.percent},
                    px: ${settings.msgBackground.user.value[i].width.px},
                },
                height: {
                    percent: ${settings.msgBackground.user.value[i].height.percent},
                    px: ${settings.msgBackground.user.value[i].height.px}
                }
            }`;
            ...
            msgBackgroundResUser += `{
                ...
                x: ${settings.msgBackground.user.value[i].x},
                y: ${settings.msgBackground.user.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    for (var i = 0; i < settings.msgBackground.sub.value.length; i++) {
        if (settings.msgBackground.sub.value[i].type === "solid") {
            msgBackgroundResSub += `{
                type: "solid",
                name: "${settings.msgBackground.sub.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.msgBackground.sub.value[i].posX.percent},
                    px: ${settings.msgBackground.sub.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.msgBackground.sub.value[i].posY.percent},
                    px: ${settings.msgBackground.sub.value[i].posY.px}
                },
                width: {
                    percent: ${settings.msgBackground.sub.value[i].width.percent},
                    px: ${settings.msgBackground.sub.value[i].width.px},
                },
                height: {
                    percent: ${settings.msgBackground.sub.value[i].height.percent},
                    px: ${settings.msgBackground.sub.value[i].height.px}
                }
            }`;
            ...
            msgBackgroundResSub += `{
                ...
                x: ${settings.msgBackground.sub.value[i].x},
                y: ${settings.msgBackground.sub.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    for (var i = 0; i < settings.msgBackground.vip.value.length; i++) {
        if (settings.msgBackground.vip.value[i].type === "solid") {
            msgBackgroundResVip += `{
                type: "solid",
                name: "${settings.msgBackground.vip.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.msgBackground.vip.value[i].posX.percent},
                    px: ${settings.msgBackground.vip.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.msgBackground.vip.value[i].posY.percent},
                    px: ${settings.msgBackground.vip.value[i].posY.px}
                },
                width: {
                    percent: ${settings.msgBackground.vip.value[i].width.percent},
                    px: ${settings.msgBackground.vip.value[i].width.px},
                },
                height: {
                    percent: ${settings.msgBackground.vip.value[i].height.percent},
                    px: ${settings.msgBackground.vip.value[i].height.px}
                }
            }`;
            ...
            msgBackgroundResVip += `{
                ...
                x: ${settings.msgBackground.vip.value[i].x},
                y: ${settings.msgBackground.vip.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    for (var i = 0; i < settings.msgBackground.mod.value.length; i++) {
        if (settings.msgBackground.mod.value[i].type === "solid") {
            msgBackgroundResMod += `{
                type: "solid",
                name: "${settings.msgBackground.mod.value[i].name}",
                colors: [
                    {
                ...
                posX: {
                    percent: ${settings.msgBackground.mod.value[i].posX.percent},
                    px: ${settings.msgBackground.mod.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.msgBackground.mod.value[i].posY.percent},
                    px: ${settings.msgBackground.mod.value[i].posY.px}
                },
                width: {
                    percent: ${settings.msgBackground.mod.value[i].width.percent},
                    px: ${settings.msgBackground.mod.value[i].width.px},
                },
                height: {
                    percent: ${settings.msgBackground.mod.value[i].height.percent},
                    px: ${settings.msgBackground.mod.value[i].height.px}
                }
            }`;
            ...
            msgBackgroundResMod += `{
                ...
                x: ${settings.msgBackground.mod.value[i].x},
                y: ${settings.msgBackground.mod.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
    ...
    var msgBackgroundMaskResUser = "";

    for (var i = 0; i < settings.msgBackgroundMask.user.value.length; i++) {
        if (settings.msgBackgroundMask.user.value[i].type === "solid") {
            msgBackgroundMaskResUser += `{
                type: "solid",
                name: "${settings.msgBackgroundMask.user.value[i].name}",
                colors: [
                    {
                        position: ${settings.msgBackgroundMask.user.value[i].colors[0].position},
                        opacity: ${settings.msgBackgroundMask.user.value[i].colors[0].opacity},
                        count: ${settings.msgBackgroundMask.user.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.msgBackgroundMask.user.value[i].posX.percent},
                    px: ${settings.msgBackgroundMask.user.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.msgBackgroundMask.user.value[i].posY.percent},
                    px: ${settings.msgBackgroundMask.user.value[i].posY.px}
                },
                width: {
                    percent: ${settings.msgBackgroundMask.user.value[i].width.percent},
                    px: ${settings.msgBackgroundMask.user.value[i].width.px},
                },
                height: {
                    percent: ${settings.msgBackgroundMask.user.value[i].height.percent},
                    px: ${settings.msgBackgroundMask.user.value[i].height.px}
                }
            }`;
        } else if (settings.msgBackgroundMask.user.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.msgBackgroundMask.user.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.msgBackgroundMask.user.value[i].colors[j].position},
                    opacity: ${settings.msgBackgroundMask.user.value[i].colors[j].opacity},
                    count: ${settings.msgBackgroundMask.user.value[i].colors[j].count}
                }`;

                if (j < settings.msgBackgroundMask.user.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            msgBackgroundMaskResUser += `{
                type: "linear",
                name: "${settings.msgBackgroundMask.user.value[i].name}",
                rotate: ${settings.msgBackgroundMask.user.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.msgBackgroundMask.user.value[i].type === "radial") {
            var colorList = "";
            ...
            msgBackgroundMaskResUser += `{
                type: "radial",
                name: "${settings.msgBackgroundMask.user.value[i].name}",
                size: "${settings.msgBackgroundMask.user.value[i].size}",
                x: ${settings.msgBackgroundMask.user.value[i].x},
                y: ${settings.msgBackgroundMask.user.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.msgBackgroundMask.user.value.length - 1) {
            msgBackgroundMaskResUser += `, `;
        }
    }

    var msgBackgroundMaskResSub = "";

    for (var i = 0; i < settings.msgBackgroundMask.sub.value.length; i++) {
        if (settings.msgBackgroundMask.sub.value[i].type === "solid") {
            msgBackgroundMaskResSub += `{
                type: "solid",
                name: "${settings.msgBackgroundMask.sub.value[i].name}",
                colors: [
                    {
                        position: ${settings.msgBackgroundMask.sub.value[i].colors[0].position},
                        opacity: ${settings.msgBackgroundMask.sub.value[i].colors[0].opacity},
                        count: ${settings.msgBackgroundMask.sub.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.msgBackgroundMask.sub.value[i].posX.percent},
                    px: ${settings.msgBackgroundMask.sub.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.msgBackgroundMask.sub.value[i].posY.percent},
                    px: ${settings.msgBackgroundMask.sub.value[i].posY.px}
                },
                width: {
                    percent: ${settings.msgBackgroundMask.sub.value[i].width.percent},
                    px: ${settings.msgBackgroundMask.sub.value[i].width.px},
                },
                height: {
                    percent: ${settings.msgBackgroundMask.sub.value[i].height.percent},
                    px: ${settings.msgBackgroundMask.sub.value[i].height.px}
                }
            }`;
        } else if (settings.msgBackgroundMask.sub.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.msgBackgroundMask.sub.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.msgBackgroundMask.sub.value[i].colors[j].position},
                    opacity: ${settings.msgBackgroundMask.sub.value[i].colors[j].opacity},
                    count: ${settings.msgBackgroundMask.sub.value[i].colors[j].count}
                }`;

                if (j < settings.msgBackgroundMask.sub.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            msgBackgroundMaskResSub += `{
                type: "linear",
                name: "${settings.msgBackgroundMask.sub.value[i].name}",
                rotate: ${settings.msgBackgroundMask.sub.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.msgBackgroundMask.sub.value[i].type === "radial") {
            var colorList = "";
            ...
            msgBackgroundMaskResSub += `{
                type: "radial",
                name: "${settings.msgBackgroundMask.sub.value[i].name}",
                size: "${settings.msgBackgroundMask.sub.value[i].size}",
                x: ${settings.msgBackgroundMask.sub.value[i].x},
                y: ${settings.msgBackgroundMask.sub.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.msgBackgroundMask.sub.value.length - 1) {
            msgBackgroundMaskResSub += `, `;
        }
    }

    var msgBackgroundMaskResVip = "";

    for (var i = 0; i < settings.msgBackgroundMask.vip.value.length; i++) {
        if (settings.msgBackgroundMask.vip.value[i].type === "solid") {
            msgBackgroundMaskResVip += `{
                type: "solid",
                name: "${settings.msgBackgroundMask.vip.value[i].name}",
                colors: [
                    {
                        position: ${settings.msgBackgroundMask.vip.value[i].colors[0].position},
                        opacity: ${settings.msgBackgroundMask.vip.value[i].colors[0].opacity},
                        count: ${settings.msgBackgroundMask.vip.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.msgBackgroundMask.vip.value[i].posX.percent},
                    px: ${settings.msgBackgroundMask.vip.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.msgBackgroundMask.vip.value[i].posY.percent},
                    px: ${settings.msgBackgroundMask.vip.value[i].posY.px}
                },
                width: {
                    percent: ${settings.msgBackgroundMask.vip.value[i].width.percent},
                    px: ${settings.msgBackgroundMask.vip.value[i].width.px},
                },
                height: {
                    percent: ${settings.msgBackgroundMask.vip.value[i].height.percent},
                    px: ${settings.msgBackgroundMask.vip.value[i].height.px}
                }
            }`;
        } else if (settings.msgBackgroundMask.vip.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.msgBackgroundMask.vip.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.msgBackgroundMask.vip.value[i].colors[j].position},
                    opacity: ${settings.msgBackgroundMask.vip.value[i].colors[j].opacity},
                    count: ${settings.msgBackgroundMask.vip.value[i].colors[j].count}
                }`;

                if (j < settings.msgBackgroundMask.vip.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            msgBackgroundMaskResVip += `{
                type: "linear",
                name: "${settings.msgBackgroundMask.vip.value[i].name}",
                rotate: ${settings.msgBackgroundMask.vip.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.msgBackgroundMask.vip.value[i].type === "radial") {
            var colorList = "";
            ...
            msgBackgroundMaskResVip += `{
                type: "radial",
                name: "${settings.msgBackgroundMask.vip.value[i].name}",
                size: "${settings.msgBackgroundMask.vip.value[i].size}",
                x: ${settings.msgBackgroundMask.vip.value[i].x},
                y: ${settings.msgBackgroundMask.vip.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.msgBackgroundMask.vip.value.length - 1) {
            msgBackgroundMaskResVip += `, `;
        }
    }

    var msgBackgroundMaskResMod = "";

    for (var i = 0; i < settings.msgBackgroundMask.mod.value.length; i++) {
        if (settings.msgBackgroundMask.mod.value[i].type === "solid") {
            msgBackgroundMaskResMod += `{
                type: "solid",
                name: "${settings.msgBackgroundMask.mod.value[i].name}",
                colors: [
                    {
                        position: ${settings.msgBackgroundMask.mod.value[i].colors[0].position},
                        opacity: ${settings.msgBackgroundMask.mod.value[i].colors[0].opacity},
                        count: ${settings.msgBackgroundMask.mod.value[i].colors[0].count}
                    }
                ],
                posX: {
                    percent: ${settings.msgBackgroundMask.mod.value[i].posX.percent},
                    px: ${settings.msgBackgroundMask.mod.value[i].posX.px}
                },
                posY: {
                    percent: ${settings.msgBackgroundMask.mod.value[i].posY.percent},
                    px: ${settings.msgBackgroundMask.mod.value[i].posY.px}
                },
                width: {
                    percent: ${settings.msgBackgroundMask.mod.value[i].width.percent},
                    px: ${settings.msgBackgroundMask.mod.value[i].width.px},
                },
                height: {
                    percent: ${settings.msgBackgroundMask.mod.value[i].height.percent},
                    px: ${settings.msgBackgroundMask.mod.value[i].height.px}
                }
            }`;
        } else if (settings.msgBackgroundMask.mod.value[i].type === "linear") {
            var colorList = "";
            
            for (var j = 0; j < settings.msgBackgroundMask.mod.value[i].colors.length; j++) {
                colorList += `{
                    position: ${settings.msgBackgroundMask.mod.value[i].colors[j].position},
                    opacity: ${settings.msgBackgroundMask.mod.value[i].colors[j].opacity},
                    count: ${settings.msgBackgroundMask.mod.value[i].colors[j].count}
                }`;

                if (j < settings.msgBackgroundMask.mod.value[i].colors.length - 1) {
                    colorList += `, `;
                }
            }
            ...
            msgBackgroundMaskResMod += `{
                type: "linear",
                name: "${settings.msgBackgroundMask.mod.value[i].name}",
                rotate: ${settings.msgBackgroundMask.mod.value[i].rotate},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        } else if (settings.msgBackgroundMask.mod.value[i].type === "radial") {
            var colorList = "";
            ...
            msgBackgroundMaskResMod += `{
                type: "radial",
                name: "${settings.msgBackgroundMask.mod.value[i].name}",
                size: "${settings.msgBackgroundMask.mod.value[i].size}",
                x: ${settings.msgBackgroundMask.mod.value[i].x},
                y: ${settings.msgBackgroundMask.mod.value[i].y},
                colors: [
                    ${colorList}
                ],
                posX: {
        ...
        if (i < settings.msgBackgroundMask.mod.value.length - 1) {
            msgBackgroundMaskResMod += `, `;
        }
    }
    ...
    for (var i = 0; i < settings.msgShadow.user.value.length; i++) {
        msgShadowResUser += `{
        x: ${settings.msgShadow.user.value[i].x},
        y: ${settings.msgShadow.user.value[i].y},
    ...
    for (var i = 0; i < settings.msgShadow.sub.value.length; i++) {
        msgShadowResSub += `{
        x: ${settings.msgShadow.sub.value[i].x},
        y: ${settings.msgShadow.sub.value[i].y},
    ...
    for (var i = 0; i < settings.msgShadow.vip.value.length; i++) {
        msgShadowResVip += `{
        x: ${settings.msgShadow.vip.value[i].x},
        y: ${settings.msgShadow.vip.value[i].y},
    ...
    for (var i = 0; i < settings.msgShadow.mod.value.length; i++) {
        msgShadowResMod += `{
        x: ${settings.msgShadow.mod.value[i].x},
        y: ${settings.msgShadow.mod.value[i].y},
    ...
    for (var i = 0; i < settings.msgFontShadow.user.value.length; i++) {
        ...
        x: ${settings.msgFontShadow.user.value[i].x},
        y: ${settings.msgFontShadow.user.value[i].y},
    ...
    for (var i = 0; i < settings.msgFontShadow.sub.value.length; i++) {
        msgFontShadowResSub += `{
        x: ${settings.msgFontShadow.sub.value[i].x},
        y: ${settings.msgFontShadow.sub.value[i].y},
    ...
    for (var i = 0; i < settings.msgFontShadow.vip.value.length; i++) {
        msgFontShadowResVip += `{
        x: ${settings.msgFontShadow.vip.value[i].x},
        y: ${settings.msgFontShadow.vip.value[i].y},
    ...
    for (var i = 0; i < settings.msgFontShadow.mod.value.length; i++) {
        msgFontShadowResMod += `{
        x: ${settings.msgFontShadow.mod.value[i].x},
        y: ${settings.msgFontShadow.mod.value[i].y},
    ...
    var colorPreset = [${presetSettingList}],
        settings = {
            ...
            cbxLayout: {
                user: {
                    value: {
                        type: "${settings.cbxLayout.user.value.type}",
                        layoutRowSizing: "${settings.cbxLayout.user.value.layoutRowSizing}"
                    },
                    active: ${settings.cbxLayout.user.active}
                },
    
                sub: {
                    value: {
                        type: "${settings.cbxLayout.sub.value.type}",
                        layoutRowSizing: "${settings.cbxLayout.sub.value.layoutRowSizing}"
                    },
                    active: ${settings.cbxLayout.sub.active}
                },
    
                vip: {
                    value: {
                        type: "${settings.cbxLayout.vip.value.type}",
                        layoutRowSizing: "${settings.cbxLayout.vip.value.layoutRowSizing}"
                    },
                    active: ${settings.cbxLayout.vip.active}
                },
    
                mod: {
                    value: {
                        type: "${settings.cbxLayout.mod.value.type}",
                        layoutRowSizing: "${settings.cbxLayout.mod.value.layoutRowSizing}"
                    },
                    active: ${settings.cbxLayout.mod.active}
                },
            },
            cbxBackground: {
                user: {
            ...
            cbxBackgroundMask: {
                user: {
                    value: [${cbxBackgroundMaskResUser}],
                    active: ${settings.cbxBackgroundMask.user.active}
                },
                sub: {
                    value: [${cbxBackgroundMaskResSub}],
                    active: ${settings.cbxBackgroundMask.sub.active}
                },
                vip: {
                    value: [${cbxBackgroundMaskResVip}],
                    active: ${settings.cbxBackgroundMask.vip.active}
                },
                mod: {
                    value: [${cbxBackgroundMaskResMod}],
                    active: ${settings.cbxBackgroundMask.mod.active}
                }
            },
            cbxBorder: {
                user: {
            ...
            usnBackgroundMask: {
                user: {
                    value: [${usnBackgroundMaskResUser}],
                    active: ${settings.usnBackgroundMask.user.active}
                },
                sub: {
                    value: [${usnBackgroundMaskResSub}],
                    active: ${settings.usnBackgroundMask.sub.active}
                },
                vip: {
                    value: [${usnBackgroundMaskResVip}],
                    active: ${settings.usnBackgroundMask.vip.active}
                },
                mod: {
                    value: [${usnBackgroundMaskResMod}],
                    active: ${settings.usnBackgroundMask.mod.active}
                }
            },
            usnBorder: {
                user: {
            ...
            usnPosition: {
                user: {
                    value: {
                        ...
                        sizingH: "${settings.usnPosition.user.value.sizingH}",
                        sizingV: "${settings.usnPosition.user.value.sizingV}"
                    },
                ...
                sub: {
                    value: {
                        ...
                        sizingH: "${settings.usnPosition.sub.value.sizingH}",
                        sizingV: "${settings.usnPosition.sub.value.sizingV}"
                    },
                ...
                vip: {
                    value: {
                        ...
                        sizingH: "${settings.usnPosition.vip.value.sizingH}",
                        sizingV: "${settings.usnPosition.vip.value.sizingV}"
                    },
                ...
                mod: {
                    value: {
                        ...
                        sizingH: "${settings.usnPosition.mod.value.sizingH}",
                        sizingV: "${settings.usnPosition.mod.value.sizingV}"
                    },
            ...
            usnFont: {
                user: {
                    value: {
                        ...
                        hAlign: "${settings.usnFont.user.value.hAlign}",
                ...
                sub: {
                    value: {
                        ...
                        hAlign: "${settings.usnFont.sub.value.hAlign}",
                ...
                vip: {
                    value: {
                        ...
                        hAlign: "${settings.usnFont.vip.value.hAlign}",
                ...
                mod: {
                    value: {
                        ...
                        hAlign: "${settings.usnFont.mod.value.hAlign}",
            ...
            msgBackgroundMask: {
                user: {
                    value: [${msgBackgroundMaskResUser}],
                    active: ${settings.msgBackgroundMask.user.active}
                },
                sub: {
                    value: [${msgBackgroundMaskResSub}],
                    active: ${settings.msgBackgroundMask.sub.active}
                },
                vip: {
                    value: [${msgBackgroundMaskResVip}],
                    active: ${settings.msgBackgroundMask.vip.active}
                },
                mod: {
                    value: [${msgBackgroundMaskResMod}],
                    active: ${settings.msgBackgroundMask.mod.active}
                }
            },
            msgBorder: {
                user: {
            ...
            msgPosition: {
                user: {
                    value: {
                        ...
                        sizingH: "${settings.msgPosition.user.value.sizingH}",
                        sizingV: "${settings.msgPosition.user.value.sizingV}"
                    },
                ...
                sub: {
                    value: {
                        ...
                        sizingH: "${settings.msgPosition.sub.value.sizingH}",
                        sizingV: "${settings.msgPosition.sub.value.sizingV}"
                    },
                ...
                vip: {
                    value: {
                        ...
                        sizingH: "${settings.msgPosition.vip.value.sizingH}",
                        sizingV: "${settings.msgPosition.vip.value.sizingV}"
                    },
                ...
                mod: {
                    value: {
                        ...
                        sizingH: "${settings.msgPosition.mod.value.sizingH}",
                        sizingV: "${settings.msgPosition.mod.value.sizingV}"
                    },
            ...
            msgFont: {
                user: {
                    value: {
                        ...
                        hAlign: "${settings.msgFont.user.value.hAlign}",
                ...
                sub: {
                    value: {
                        ...
                        hAlign: "${settings.msgFont.sub.value.hAlign}",
                ...
                vip: {
                    value: {
                        ...
                        hAlign: "${settings.msgFont.vip.value.hAlign}",
                ...
                mod: {
                    value: {
                        ...
                        hAlign: "${settings.msgFont.mod.value.hAlign}",
            ...
            othBadges: {
                user: {
                    value: {
                        gap: ${settings.othBadges.user.value.gap}, // In px
                        size: {
                            height: ${settings.othBadges.user.value.size.height}, // In px
                            auto: ${settings.othBadges.user.value.size.auto}
                        }
                    },
                    active: ${settings.othBadges.user.active}
                },
                sub: {
                    value: {
                        gap: ${settings.othBadges.sub.value.gap}, // In px
                        size: {
                            height: ${settings.othBadges.sub.value.size.height}, // In px
                            auto: ${settings.othBadges.sub.value.size.auto}
                        }
                    },
                    active: ${settings.othBadges.sub.active}
                },
                vip: {
                    value: {
                        gap: ${settings.othBadges.vip.value.gap}, // In px
                        size: {
                            height: ${settings.othBadges.vip.value.size.height}, // In px
                            auto: ${settings.othBadges.vip.value.size.auto}
                        }
                    },
                    active: ${settings.othBadges.vip.active}
                },
                mod: {
                    value: {
                        gap: ${settings.othBadges.mod.value.gap}, // In px
                        size: {
                            height: ${settings.othBadges.mod.value.size.height}, // In px
                            auto: ${settings.othBadges.mod.value.size.auto}
                        }
                    },
                    active: ${settings.othBadges.mod.active}
                }
            },
            othHyphens: {
                active: ${settings.othHyphens.active},
                lang: "${settings.othHyphens.lang}"
            },
            anmTiming: {
                user: {
                    value: {
                    ...
                    value: [
                        {
                            ...
                            type: "curve",
                            value: "${settings.anmShow.user.value[7].value}"
                        }
                    ],
                    ...
                    value: [
                        {
                            ...
                            type: "curve",
                            value: "${settings.anmShow.sub.value[7].value}"
                        }
                    ],
                    ...
                    value: [
                        {
                            ...
                            type: "curve",
                            value: "${settings.anmShow.vip.value[7].value}"
                        }
                    ],
                    ...
                    value: [
                        {
                            ...
                            type: "curve",
                            value: "${settings.anmShow.mod.value[7].value}"
                        }
                    ],
                    ...
                    value: [
                        {
                            ...
                            type: "curve",
                            value: "${settings.anmHide.user.value[7].value}"
                        }
                    ],
                    ...
                    value: [
                        {
                            ...
                            type: "curve",
                            value: "${settings.anmHide.sub.value[7].value}"
                        }
                    ],
                    ...
                    value: [
                        {
                            ...
                            type: "curve",
                            value: "${settings.anmHide.vip.value[7].value}"
                        }
                    ],
                    ...
                    value: [
                        {
                            ...
                            type: "curve",
                            value: "${settings.anmHide.mod.value[7].value}"
                        }
                    ],
